// Code generated by make_resources.go; DO NOT EDIT.
package locales

import (
	"github.com/razor-1/cldr"
	"github.com/razor-1/cldr/resources/currency"
	"github.com/razor-1/cldr/resources/language"
	"github.com/razor-1/cldr/resources/territory"
)

func Get_ms_BN() *cldr.Locale {
	return &cldr.Locale{
		Locale:   "ms_BN",
		Calendar: cldr.Calendar{Formats: cldr.CalendarFormats{Date: cldr.CalendarDateFormat{Full: "dd MMMM y", Long: "d MMMM y", Medium: "d MMM y", Short: "d/MM/yy"}, Time: cldr.CalendarDateFormat{Full: "h:mm:ss\u202fa zzzz", Long: "h:mm:ss\u202fa z", Medium: "h:mm:ss\u202fa", Short: "h:mm\u202fa"}, DateTime: cldr.CalendarDateFormat{Full: "{1} {0}", Long: "{1} {0}", Medium: "{1}, {0}", Short: "{1}, {0}"}, GMT: "GMT{0}"}, FormatNames: cldr.CalendarFormatNames{Months: cldr.CalendarMonthFormatNames{Abbreviated: cldr.CalendarMonthFormatNameValue{Jan: "Jan", Feb: "Feb", Mar: "Mac", Apr: "Apr", May: "Mei", Jun: "Jun", Jul: "Jul", Aug: "Ogo", Sep: "Sep", Oct: "Okt", Nov: "Nov", Dec: "Dis"}, Narrow: cldr.CalendarMonthFormatNameValue{Jan: "J", Feb: "F", Mar: "M", Apr: "A", May: "M", Jun: "J", Jul: "J", Aug: "O", Sep: "S", Oct: "O", Nov: "N", Dec: "D"}, Short: cldr.CalendarMonthFormatNameValue{Jan: "", Feb: "", Mar: "", Apr: "", May: "", Jun: "", Jul: "", Aug: "", Sep: "", Oct: "", Nov: "", Dec: ""}, Wide: cldr.CalendarMonthFormatNameValue{Jan: "Januari", Feb: "Februari", Mar: "Mac", Apr: "April", May: "Mei", Jun: "Jun", Jul: "Julai", Aug: "Ogos", Sep: "September", Oct: "Oktober", Nov: "November", Dec: "Disember"}}, Days: cldr.CalendarDayFormatNames{Abbreviated: cldr.CalendarDayFormatNameValue{Sun: "Ahd", Mon: "Isn", Tue: "Sel", Wed: "Rab", Thu: "Kha", Fri: "Jum", Sat: "Sab"}, Narrow: cldr.CalendarDayFormatNameValue{Sun: "A", Mon: "I", Tue: "S", Wed: "R", Thu: "K", Fri: "J", Sat: "S"}, Short: cldr.CalendarDayFormatNameValue{Sun: "Ah", Mon: "Is", Tue: "Se", Wed: "Ra", Thu: "Kh", Fri: "Ju", Sat: "Sa"}, Wide: cldr.CalendarDayFormatNameValue{Sun: "Ahad", Mon: "Isnin", Tue: "Selasa", Wed: "Rabu", Thu: "Khamis", Fri: "Jumaat", Sat: "Sabtu"}}, Periods: cldr.CalendarPeriodFormatNames{Abbreviated: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Narrow: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Short: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Wide: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}}}},
		Number: cldr.Number{
			Symbols: cldr.Symbols{Decimal: ",", Group: ".", Negative: "-", Percent: "%", PerMille: "‰"},
			Formats: cldr.NumberFormats{Decimal: "#,##0.###", Currency: "¤\u00a0#,##0.00", CurrencyAccounting: "¤#,##0.00;(¤#,##0.00)", Percent: ""},
			Currencies: cldr.Currencies{
				currency.AED: cldr.Currency{DisplayName: "Dirham Emiriah Arab Bersatu", Symbol: ""},
				currency.AFN: cldr.Currency{DisplayName: "Afghani Afghanistan", Symbol: "؋"},
				currency.ALL: cldr.Currency{DisplayName: "Lek Albania", Symbol: ""},
				currency.AMD: cldr.Currency{DisplayName: "Dram Armenia", Symbol: "֏"},
				currency.ANG: cldr.Currency{DisplayName: "Guilder Antillen Belanda", Symbol: ""},
				currency.AOA: cldr.Currency{DisplayName: "Kwanza Angola", Symbol: "Kz"},
				currency.ARS: cldr.Currency{DisplayName: "Peso Argentina", Symbol: "$"},
				currency.AUD: cldr.Currency{DisplayName: "Dolar Australia", Symbol: "A$"},
				currency.AWG: cldr.Currency{DisplayName: "Florin Aruba", Symbol: ""},
				currency.AZN: cldr.Currency{DisplayName: "Manat Azerbaijan", Symbol: "₼"},
				currency.BAM: cldr.Currency{DisplayName: "Mark Boleh Tukar Bosnia-Herzegovina", Symbol: "KM"},
				currency.BBD: cldr.Currency{DisplayName: "Dolar Barbados", Symbol: "$"},
				currency.BDT: cldr.Currency{DisplayName: "Taka Bangladesh", Symbol: "৳"},
				currency.BGN: cldr.Currency{DisplayName: "Lev Bulgaria", Symbol: ""},
				currency.BHD: cldr.Currency{DisplayName: "Dinar Bahrain", Symbol: ""},
				currency.BIF: cldr.Currency{DisplayName: "Franc Burundi", Symbol: ""},
				currency.BMD: cldr.Currency{DisplayName: "Dolar Bermuda", Symbol: "$"},
				currency.BND: cldr.Currency{DisplayName: "Dolar Brunei", Symbol: "$"},
				currency.BOB: cldr.Currency{DisplayName: "Boliviano Bolivia", Symbol: "Bs"},
				currency.BRL: cldr.Currency{DisplayName: "Real Brazil", Symbol: "R$"},
				currency.BSD: cldr.Currency{DisplayName: "Dolar Bahamas", Symbol: "$"},
				currency.BTN: cldr.Currency{DisplayName: "Ngultrum Bhutan", Symbol: ""},
				currency.BWP: cldr.Currency{DisplayName: "Pula Botswana", Symbol: "P"},
				currency.BYN: cldr.Currency{DisplayName: "Rubel Belarus baharu", Symbol: "р."},
				currency.BYR: cldr.Currency{DisplayName: "Rubel Belarus (2000–2016)", Symbol: ""},
				currency.BZD: cldr.Currency{DisplayName: "Dolar Belize", Symbol: "$"},
				currency.CAD: cldr.Currency{DisplayName: "Dolar Kanada", Symbol: "CAD"},
				currency.CDF: cldr.Currency{DisplayName: "Franc Congo", Symbol: ""},
				currency.CHF: cldr.Currency{DisplayName: "Franc Switzerland", Symbol: ""},
				currency.CLP: cldr.Currency{DisplayName: "Peso Chile", Symbol: "$"},
				currency.CNH: cldr.Currency{DisplayName: "Yuan China (luar pesisir)", Symbol: ""},
				currency.CNY: cldr.Currency{DisplayName: "Yuan Cina", Symbol: "CN¥"},
				currency.COP: cldr.Currency{DisplayName: "Peso Colombia", Symbol: "$"},
				currency.CRC: cldr.Currency{DisplayName: "Colon Costa Rica", Symbol: "₡"},
				currency.CUC: cldr.Currency{DisplayName: "Peso Boleh Tukar Cuba", Symbol: "$"},
				currency.CUP: cldr.Currency{DisplayName: "Peso Cuba", Symbol: "$"},
				currency.CVE: cldr.Currency{DisplayName: "Escudo Tanjung Verde", Symbol: ""},
				currency.CZK: cldr.Currency{DisplayName: "Koruna Republik Czech", Symbol: "Kč"},
				currency.DJF: cldr.Currency{DisplayName: "Franc Djibouti", Symbol: ""},
				currency.DKK: cldr.Currency{DisplayName: "Krone Denmark", Symbol: "kr"},
				currency.DOP: cldr.Currency{DisplayName: "Peso Dominican", Symbol: "$"},
				currency.DZD: cldr.Currency{DisplayName: "Dinar Algeria", Symbol: ""},
				currency.EGP: cldr.Currency{DisplayName: "Paun Mesir", Symbol: "E£"},
				currency.ERN: cldr.Currency{DisplayName: "Nakfa Eritrea", Symbol: ""},
				currency.ESP: cldr.Currency{DisplayName: "", Symbol: "₧"},
				currency.ETB: cldr.Currency{DisplayName: "Birr Ethiopia", Symbol: ""},
				currency.EUR: cldr.Currency{DisplayName: "Euro", Symbol: "€"},
				currency.FJD: cldr.Currency{DisplayName: "Dolar Fiji", Symbol: "$"},
				currency.FKP: cldr.Currency{DisplayName: "Paun Kepulauan Falkland", Symbol: "£"},
				currency.GBP: cldr.Currency{DisplayName: "Paun British", Symbol: "£"},
				currency.GEL: cldr.Currency{DisplayName: "Lari Georgia", Symbol: "₾"},
				currency.GHS: cldr.Currency{DisplayName: "Cedi Ghana", Symbol: "GH₵"},
				currency.GIP: cldr.Currency{DisplayName: "Paun Gibraltar", Symbol: "£"},
				currency.GMD: cldr.Currency{DisplayName: "Dalasi Gambia", Symbol: ""},
				currency.GNF: cldr.Currency{DisplayName: "Franc Guinea", Symbol: "FG"},
				currency.GTQ: cldr.Currency{DisplayName: "Quetzal Guatemala", Symbol: "Q"},
				currency.GYD: cldr.Currency{DisplayName: "Dolar Guyana", Symbol: "$"},
				currency.HKD: cldr.Currency{DisplayName: "Dolar Hong Kong", Symbol: "HK$"},
				currency.HNL: cldr.Currency{DisplayName: "Lempira Honduras", Symbol: "L"},
				currency.HRK: cldr.Currency{DisplayName: "Kuna Croatia", Symbol: "kn"},
				currency.HTG: cldr.Currency{DisplayName: "Gourde Haiti", Symbol: ""},
				currency.HUF: cldr.Currency{DisplayName: "Forint Hungary", Symbol: "Ft"},
				currency.IDR: cldr.Currency{DisplayName: "Rupiah Indonesia", Symbol: "Rp"},
				currency.ILS: cldr.Currency{DisplayName: "Syekel Baharu Israel", Symbol: "₪"},
				currency.INR: cldr.Currency{DisplayName: "Rupee India", Symbol: "₹"},
				currency.IQD: cldr.Currency{DisplayName: "Dinar Iraq", Symbol: ""},
				currency.IRR: cldr.Currency{DisplayName: "Rial Iran", Symbol: ""},
				currency.ISK: cldr.Currency{DisplayName: "Krona Iceland", Symbol: "kr"},
				currency.JMD: cldr.Currency{DisplayName: "Dolar Jamaica", Symbol: "$"},
				currency.JOD: cldr.Currency{DisplayName: "Dinar Jordan", Symbol: ""},
				currency.JPY: cldr.Currency{DisplayName: "Yen Jepun", Symbol: "JP¥"},
				currency.KES: cldr.Currency{DisplayName: "Syiling Kenya", Symbol: ""},
				currency.KGS: cldr.Currency{DisplayName: "Som Kyrgystani", Symbol: "⃀"},
				currency.KHR: cldr.Currency{DisplayName: "Riel Kemboja", Symbol: "៛"},
				currency.KMF: cldr.Currency{DisplayName: "Franc Comoria", Symbol: "CF"},
				currency.KPW: cldr.Currency{DisplayName: "Won Korea Utara", Symbol: "₩"},
				currency.KRW: cldr.Currency{DisplayName: "Won Korea Selatan", Symbol: "₩"},
				currency.KWD: cldr.Currency{DisplayName: "Dinar Kuwait", Symbol: ""},
				currency.KYD: cldr.Currency{DisplayName: "Dolar Kepulauan Cayman", Symbol: "$"},
				currency.KZT: cldr.Currency{DisplayName: "Tenge Kazakhstan", Symbol: "₸"},
				currency.LAK: cldr.Currency{DisplayName: "Kip Laos", Symbol: "₭"},
				currency.LBP: cldr.Currency{DisplayName: "Paun Lubnan", Symbol: "L£"},
				currency.LKR: cldr.Currency{DisplayName: "Rupee Sri Lanka", Symbol: "Rs"},
				currency.LRD: cldr.Currency{DisplayName: "Dolar Liberia", Symbol: "$"},
				currency.LSL: cldr.Currency{DisplayName: "Loti Lesotho", Symbol: ""},
				currency.LTL: cldr.Currency{DisplayName: "Litas Lithuania", Symbol: "Lt"},
				currency.LVL: cldr.Currency{DisplayName: "Lats Latvia", Symbol: "Ls"},
				currency.LYD: cldr.Currency{DisplayName: "Dinar Libya", Symbol: ""},
				currency.MAD: cldr.Currency{DisplayName: "Dirham Maghribi", Symbol: ""},
				currency.MDL: cldr.Currency{DisplayName: "Leu Moldova", Symbol: ""},
				currency.MGA: cldr.Currency{DisplayName: "Ariary Malagasy", Symbol: "Ar"},
				currency.MGF: cldr.Currency{DisplayName: "Franc Malagasy", Symbol: ""},
				currency.MKD: cldr.Currency{DisplayName: "Denar Macedonia", Symbol: ""},
				currency.MMK: cldr.Currency{DisplayName: "Kyat Myanma", Symbol: "K"},
				currency.MNT: cldr.Currency{DisplayName: "Tugrik Mongolia", Symbol: "₮"},
				currency.MOP: cldr.Currency{DisplayName: "Pataca Macau", Symbol: ""},
				currency.MRO: cldr.Currency{DisplayName: "Ouguiya Mauritania (1973–2017)", Symbol: ""},
				currency.MRU: cldr.Currency{DisplayName: "Ouguiya Mauritania", Symbol: ""},
				currency.MUR: cldr.Currency{DisplayName: "Rupee Mauritius", Symbol: "Rs"},
				currency.MVR: cldr.Currency{DisplayName: "Rufiyaa Maldives", Symbol: ""},
				currency.MWK: cldr.Currency{DisplayName: "Kwacha Malawi", Symbol: ""},
				currency.MXN: cldr.Currency{DisplayName: "Peso Mexico", Symbol: "MXN"},
				currency.MYR: cldr.Currency{DisplayName: "Ringgit Malaysia", Symbol: "RM"},
				currency.MZE: cldr.Currency{DisplayName: "Escudo Mozambique", Symbol: ""},
				currency.MZM: cldr.Currency{DisplayName: "Metical Mozambique (1980–2006)", Symbol: ""},
				currency.MZN: cldr.Currency{DisplayName: "Metikal Mozambique", Symbol: ""},
				currency.NAD: cldr.Currency{DisplayName: "Dolar Namibia", Symbol: "$"},
				currency.NGN: cldr.Currency{DisplayName: "Naira Nigeria", Symbol: "₦"},
				currency.NIO: cldr.Currency{DisplayName: "Cordoba Nicaragua", Symbol: "C$"},
				currency.NOK: cldr.Currency{DisplayName: "Krone Norway", Symbol: "kr"},
				currency.NPR: cldr.Currency{DisplayName: "Rupee Nepal", Symbol: "Rs"},
				currency.NZD: cldr.Currency{DisplayName: "Dolar New Zealand", Symbol: "NZ$"},
				currency.OMR: cldr.Currency{DisplayName: "Rial Oman", Symbol: ""},
				currency.PAB: cldr.Currency{DisplayName: "Balboa Panama", Symbol: ""},
				currency.PEN: cldr.Currency{DisplayName: "Sol Peru", Symbol: ""},
				currency.PGK: cldr.Currency{DisplayName: "Kina Papua New Guinea", Symbol: ""},
				currency.PHP: cldr.Currency{DisplayName: "Peso Filipina", Symbol: "PHP"},
				currency.PKR: cldr.Currency{DisplayName: "Rupee Pakistan", Symbol: "Rs"},
				currency.PLN: cldr.Currency{DisplayName: "Zloty Poland", Symbol: "zł"},
				currency.PYG: cldr.Currency{DisplayName: "Guarani Paraguay", Symbol: "₲"},
				currency.QAR: cldr.Currency{DisplayName: "Rial Qatar", Symbol: ""},
				currency.RHD: cldr.Currency{DisplayName: "Dolar Rhodesia", Symbol: ""},
				currency.RON: cldr.Currency{DisplayName: "Leu Romania", Symbol: "lei"},
				currency.RSD: cldr.Currency{DisplayName: "Dinar Serbia", Symbol: ""},
				currency.RUB: cldr.Currency{DisplayName: "Rubel Rusia", Symbol: "₽"},
				currency.RWF: cldr.Currency{DisplayName: "Franc Rwanda", Symbol: "RF"},
				currency.SAR: cldr.Currency{DisplayName: "Riyal Saudi", Symbol: ""},
				currency.SBD: cldr.Currency{DisplayName: "Dolar Kepulauan Solomon", Symbol: "$"},
				currency.SCR: cldr.Currency{DisplayName: "Rupee Seychelles", Symbol: ""},
				currency.SDG: cldr.Currency{DisplayName: "Paun Sudan", Symbol: ""},
				currency.SEK: cldr.Currency{DisplayName: "Krona Sweden", Symbol: "kr"},
				currency.SGD: cldr.Currency{DisplayName: "Dolar Singapura", Symbol: "$"},
				currency.SHP: cldr.Currency{DisplayName: "Paun Saint Helena", Symbol: "£"},
				currency.SLE: cldr.Currency{DisplayName: "Leone Sierra Leone", Symbol: ""},
				currency.SLL: cldr.Currency{DisplayName: "Leone Sierra Leone (1964—2022)", Symbol: ""},
				currency.SOS: cldr.Currency{DisplayName: "Syiling Somali", Symbol: ""},
				currency.SRD: cldr.Currency{DisplayName: "Dolar Surinam", Symbol: "$"},
				currency.SSP: cldr.Currency{DisplayName: "Paun Sudan selatan", Symbol: "£"},
				currency.STD: cldr.Currency{DisplayName: "Dobra Sao Tome dan Principe (1977–2017)", Symbol: ""},
				currency.STN: cldr.Currency{DisplayName: "Dobra Sao Tome dan Principe", Symbol: "Db"},
				currency.SYP: cldr.Currency{DisplayName: "Paun Syria", Symbol: "£"},
				currency.SZL: cldr.Currency{DisplayName: "Lilangeni Swazi", Symbol: ""},
				currency.THB: cldr.Currency{DisplayName: "Baht Thai", Symbol: "฿"},
				currency.TJS: cldr.Currency{DisplayName: "Somoni Tajikistan", Symbol: ""},
				currency.TMT: cldr.Currency{DisplayName: "Manat Turkmenistan", Symbol: ""},
				currency.TND: cldr.Currency{DisplayName: "Dinar Tunisia", Symbol: ""},
				currency.TOP: cldr.Currency{DisplayName: "Pa’anga Tonga", Symbol: "T$"},
				currency.TRY: cldr.Currency{DisplayName: "Lira Turki", Symbol: "₺"},
				currency.TTD: cldr.Currency{DisplayName: "Dolar Trinidad dan Tobago", Symbol: "$"},
				currency.TWD: cldr.Currency{DisplayName: "Dolar Taiwan Baru", Symbol: "NT$"},
				currency.TZS: cldr.Currency{DisplayName: "Syiling Tanzania", Symbol: ""},
				currency.UAH: cldr.Currency{DisplayName: "Hryvnia Ukraine", Symbol: "₴"},
				currency.UGS: cldr.Currency{DisplayName: "Shilling Uganda (1966–1987)", Symbol: ""},
				currency.UGX: cldr.Currency{DisplayName: "Syiling Uganda", Symbol: ""},
				currency.USD: cldr.Currency{DisplayName: "Dolar AS", Symbol: "USD"},
				currency.UYU: cldr.Currency{DisplayName: "Peso Uruguay", Symbol: "$"},
				currency.UZS: cldr.Currency{DisplayName: "Som Uzbekistan", Symbol: ""},
				currency.VEF: cldr.Currency{DisplayName: "Bolivar Venezuela (2008–2018)", Symbol: "Bs"},
				currency.VES: cldr.Currency{DisplayName: "Bolivar Venezuela", Symbol: ""},
				currency.VND: cldr.Currency{DisplayName: "Dong Vietnam", Symbol: "₫"},
				currency.VUV: cldr.Currency{DisplayName: "Vatu Vanuatu", Symbol: ""},
				currency.WST: cldr.Currency{DisplayName: "Tala Samoa", Symbol: ""},
				currency.XAF: cldr.Currency{DisplayName: "Franc CFA BEAC", Symbol: "FCFA"},
				currency.XCD: cldr.Currency{DisplayName: "Dolar Caribbean Timur", Symbol: "EC$"},
				currency.XCG: cldr.Currency{DisplayName: "", Symbol: "Cg."},
				currency.XOF: cldr.Currency{DisplayName: "Franc CFA BCEAO", Symbol: "F\u202fCFA"},
				currency.XPF: cldr.Currency{DisplayName: "Franc CFP", Symbol: "CFPF"},
				currency.XXX: cldr.Currency{DisplayName: "Mata Wang Tidak Diketahui", Symbol: "¤"},
				currency.YER: cldr.Currency{DisplayName: "Rial Yaman", Symbol: ""},
				currency.ZAR: cldr.Currency{DisplayName: "Rand Afrika Selatan", Symbol: "R"},
				currency.ZMK: cldr.Currency{DisplayName: "Kwacha Zambia (1968–2012)", Symbol: ""},
				currency.ZMW: cldr.Currency{DisplayName: "Kwacha Zambia", Symbol: "ZK"},
				currency.ZWD: cldr.Currency{DisplayName: "Dolar Zimbabwe (1980–2008)", Symbol: ""},
				currency.ZWL: cldr.Currency{DisplayName: "Dolar Zimbabwe (2009)", Symbol: ""},
				currency.ZWR: cldr.Currency{DisplayName: "Dolar Zimbabwe (2008)", Symbol: ""},
			},
		},
		Display: cldr.LocaleDisplayPattern{Pattern: "{0} ({1})", Separator: "{0}, {1}", KeyTypePattern: "{0}: {1}"},
		Languages: cldr.Languages{
			language.AA:      "Afar",
			language.AB:      "Abkhazia",
			language.ACE:     "Aceh",
			language.ACH:     "Akoli",
			language.ADA:     "Adangme",
			language.ADY:     "Adyghe",
			language.AE:      "Avestan",
			language.AEB:     "Arab Tunisia",
			language.AF:      "Afrikaans",
			language.AGQ:     "Aghem",
			language.AIN:     "Ainu",
			language.AK:      "Akan",
			language.ALE:     "Aleut",
			language.ALT:     "Altai Selatan",
			language.AM:      "Amharic",
			language.AN:      "Aragon",
			language.ANN:     "Obolo",
			language.ANP:     "Angika",
			language.AR:      "Arab",
			language.AR_001:  "Arab Standard Moden",
			language.ARN:     "Mapuche",
			language.ARP:     "Arapaho",
			language.ARQ:     "Arab Algeria",
			language.ARS:     "Arab Najdi",
			language.ARY:     "Arab Maghribi",
			language.ARZ:     "Arab Mesir",
			language.AS:      "Assam",
			language.ASA:     "Asu",
			language.AST:     "Asturia",
			language.ATJ:     "Atikamekw",
			language.AV:      "Avaric",
			language.AWA:     "Awadhi",
			language.AY:      "Aymara",
			language.AZ:      "Azeri",
			language.BA:      "Bashkir",
			language.BAL:     "Baluchi",
			language.BAN:     "Bali",
			language.BAS:     "Basaa",
			language.BAX:     "Bamun",
			language.BBJ:     "Ghomala",
			language.BE:      "Belarus",
			language.BEJ:     "Beja",
			language.BEM:     "Bemba",
			language.BEZ:     "Bena",
			language.BFD:     "Bafut",
			language.BG:      "Bulgaria",
			language.BGC:     "Haryanvi",
			language.BGN:     "Balochi Barat",
			language.BHO:     "Bhojpuri",
			language.BI:      "Bislama",
			language.BIN:     "Bini",
			language.BKM:     "Kom",
			language.BLA:     "Siksika",
			language.BLO:     "Anii",
			language.BM:      "Bambara",
			language.BN:      "Benggali",
			language.BO:      "Tibet",
			language.BPY:     "Bishnupriya",
			language.BR:      "Breton",
			language.BRH:     "Brahui",
			language.BRX:     "Bodo",
			language.BS:      "Bosnia",
			language.BSS:     "Akoose",
			language.BUA:     "Buriat",
			language.BUG:     "Bugis",
			language.BUM:     "Bulu",
			language.BYN:     "Blin",
			language.BYV:     "Medumba",
			language.CA:      "Catalonia",
			language.CAY:     "Cayuga",
			language.CCP:     "Chakma",
			language.CE:      "Chechen",
			language.CEB:     "Cebuano",
			language.CGG:     "Chiga",
			language.CH:      "Chamorro",
			language.CHK:     "Chukese",
			language.CHM:     "Mari",
			language.CHO:     "Choctaw",
			language.CHP:     "Chipewyan",
			language.CHR:     "Cherokee",
			language.CHY:     "Cheyenne",
			language.CKB:     "Kurdi, Sorani",
			language.CLC:     "Chilcotin",
			language.CO:      "Corsica",
			language.COP:     "Coptic",
			language.CRG:     "Michif",
			language.CRH:     "Turki Krimea",
			language.CRJ:     "Cree Tenggara",
			language.CRK:     "Plains Cree",
			language.CRL:     "Timur Laut Cree",
			language.CRM:     "Moose Cree",
			language.CRR:     "Carolina Algonquian",
			language.CRS:     "Perancis Seselwa Creole",
			language.CS:      "Czech",
			language.CSW:     "Swampy Cree",
			language.CU:      "Slavik Gereja",
			language.CV:      "Chuvash",
			language.CY:      "Wales",
			language.DA:      "Denmark",
			language.DAK:     "Dakota",
			language.DAR:     "Dargwa",
			language.DAV:     "Taita",
			language.DE:      "Jerman",
			language.DE_AT:   "Jerman Austria",
			language.DE_CH:   "Jerman Halus Switzerland",
			language.DGR:     "Dogrib",
			language.DJE:     "Zarma",
			language.DOI:     "Dogri",
			language.DSB:     "Sorbian Rendah",
			language.DUA:     "Duala",
			language.DV:      "Divehi",
			language.DYO:     "Jola-Fonyi",
			language.DZ:      "Dzongkha",
			language.DZG:     "Dazaga",
			language.EBU:     "Embu",
			language.EE:      "Ewe",
			language.EFI:     "Efik",
			language.EKA:     "Ekajuk",
			language.EL:      "Greek",
			language.EN:      "Inggeris",
			language.EN_AU:   "Inggeris Australia",
			language.EN_CA:   "Inggeris Kanada",
			language.EN_GB:   "Inggeris U.K.",
			language.EN_US:   "Inggeris A.S.",
			language.EO:      "Esperanto",
			language.ES:      "Sepanyol",
			language.ES_419:  "Sepanyol Amerika Latin",
			language.ES_ES:   "Sepanyol Eropah",
			language.ES_MX:   "Sepanyol Mexico",
			language.ET:      "Estonia",
			language.EU:      "Basque",
			language.EWO:     "Ewondo",
			language.FA:      "Parsi",
			language.FA_AF:   "Dari",
			language.FF:      "Fulah",
			language.FI:      "Finland",
			language.FIL:     "Filipina",
			language.FJ:      "Fiji",
			language.FO:      "Faroe",
			language.FON:     "Fon",
			language.FR:      "Perancis",
			language.FR_CA:   "Perancis Kanada",
			language.FR_CH:   "Perancis Switzerland",
			language.FRC:     "Perancis Cajun",
			language.FRR:     "Frisian Utara",
			language.FUR:     "Friulian",
			language.FY:      "Frisian Barat",
			language.GA:      "Ireland",
			language.GAA:     "Ga",
			language.GAG:     "Gagauz",
			language.GAN:     "Cina Gan",
			language.GBA:     "Gbaya",
			language.GBZ:     "Zoroastrian Dari",
			language.GD:      "Scots Gaelic",
			language.GEZ:     "Geez",
			language.GIL:     "Kiribati",
			language.GL:      "Galicia",
			language.GLK:     "Gilaki",
			language.GN:      "Guarani",
			language.GOR:     "Gorontalo",
			language.GRC:     "Greek Purba",
			language.GSW:     "Jerman Switzerland",
			language.GU:      "Gujarat",
			language.GUZ:     "Gusii",
			language.GV:      "Manx",
			language.GWI:     "Gwichʼin",
			language.HA:      "Hausa",
			language.HAI:     "Haida",
			language.HAK:     "Cina Hakka",
			language.HAW:     "Hawaii",
			language.HAX:     "Haida Selatan",
			language.HE:      "Ibrani",
			language.HI:      "Hindi",
			language.HI_LATN: "Hinglish",
			language.HIL:     "Hiligaynon",
			language.HMN:     "Hmong",
			language.HR:      "Croatia",
			language.HSB:     "Sorbian Atas",
			language.HSN:     "Cina Xiang",
			language.HT:      "Kreol Haiti",
			language.HU:      "Hungary",
			language.HUP:     "Hupa",
			language.HUR:     "Halkomelem",
			language.HY:      "Armenia",
			language.HZ:      "Herero",
			language.IA:      "Interlingua",
			language.IBA:     "Iban",
			language.IBB:     "Ibibio",
			language.ID:      "Indonesia",
			language.IE:      "Interlingue",
			language.IG:      "Igbo",
			language.II:      "Sichuan Yi",
			language.IKT:     "Inuktitut Kanada Barat",
			language.ILO:     "Iloko",
			language.INH:     "Ingush",
			language.IO:      "Ido",
			language.IS:      "Iceland",
			language.IT:      "Itali",
			language.IU:      "Inuktitut",
			language.JA:      "Jepun",
			language.JBO:     "Lojban",
			language.JGO:     "Ngomba",
			language.JMC:     "Machame",
			language.JV:      "Jawa",
			language.KA:      "Georgia",
			language.KAB:     "Kabyle",
			language.KAC:     "Kachin",
			language.KAJ:     "Jju",
			language.KAM:     "Kamba",
			language.KBD:     "Kabardia",
			language.KBL:     "Kanembu",
			language.KCG:     "Tyap",
			language.KDE:     "Makonde",
			language.KEA:     "Kabuverdianu",
			language.KFO:     "Koro",
			language.KG:      "Kongo",
			language.KGP:     "Kaingang",
			language.KHA:     "Khasi",
			language.KHQ:     "Koyra Chiini",
			language.KHW:     "Khowar",
			language.KI:      "Kikuya",
			language.KJ:      "Kuanyama",
			language.KK:      "Kazakhstan",
			language.KKJ:     "Kako",
			language.KL:      "Kalaallisut",
			language.KLN:     "Kalenjin",
			language.KM:      "Khmer",
			language.KMB:     "Kimbundu",
			language.KN:      "Kannada",
			language.KO:      "Korea",
			language.KOI:     "Komi-Permyak",
			language.KOK:     "Konkani",
			language.KPE:     "Kpelle",
			language.KR:      "Kanuri",
			language.KRC:     "Karachay-Balkar",
			language.KRL:     "Karelian",
			language.KRU:     "Kurukh",
			language.KS:      "Kashmir",
			language.KSB:     "Shambala",
			language.KSF:     "Bafia",
			language.KSH:     "Colognian",
			language.KU:      "Kurdish",
			language.KUM:     "Kumyk",
			language.KV:      "Komi",
			language.KW:      "Cornish",
			language.KWK:     "Kwak’wala",
			language.KXV:     "Kuvi",
			language.KY:      "Kirghiz",
			language.LA:      "Latin",
			language.LAD:     "Ladino",
			language.LAG:     "Langi",
			language.LAH:     "Lahnda",
			language.LB:      "Luxembourg",
			language.LEZ:     "Lezghian",
			language.LG:      "Ganda",
			language.LI:      "Limburgish",
			language.LIJ:     "Liguria",
			language.LIL:     "Lillooet",
			language.LKT:     "Lakota",
			language.LMO:     "Lombard",
			language.LN:      "Lingala",
			language.LO:      "Laos",
			language.LOU:     "Kreol Louisiana",
			language.LOZ:     "Lozi",
			language.LRC:     "Luri Utara",
			language.LSM:     "Saamia",
			language.LT:      "Lithuania",
			language.LU:      "Luba-Katanga",
			language.LUA:     "Luba-Lulua",
			language.LUN:     "Lunda",
			language.LUO:     "Luo",
			language.LUS:     "Mizo",
			language.LUY:     "Luyia",
			language.LV:      "Latvia",
			language.MAD:     "Madura",
			language.MAF:     "Mafa",
			language.MAG:     "Magahi",
			language.MAI:     "Maithili",
			language.MAK:     "Makasar",
			language.MAS:     "Masai",
			language.MDE:     "Maba",
			language.MDF:     "Moksha",
			language.MEN:     "Mende",
			language.MER:     "Meru",
			language.MFE:     "Morisyen",
			language.MG:      "Malagasy",
			language.MGH:     "Makhuwa-Meetto",
			language.MGO:     "Meta’",
			language.MH:      "Marshall",
			language.MI:      "Maori",
			language.MIC:     "Micmac",
			language.MIN:     "Minangkabau",
			language.MK:      "Macedonia",
			language.ML:      "Malayalam",
			language.MN:      "Mongolia",
			language.MNI:     "Manipuri",
			language.MOE:     "Innu-aimun",
			language.MOH:     "Mohawk",
			language.MOS:     "Mossi",
			language.MR:      "Marathi",
			language.MS:      "Melayu",
			language.MT:      "Malta",
			language.MUA:     "Mundang",
			language.MUL:     "Pelbagai Bahasa",
			language.MUS:     "Creek",
			language.MWL:     "Mirandese",
			language.MY:      "Burma",
			language.MYE:     "Myene",
			language.MYV:     "Erzya",
			language.MZN:     "Mazanderani",
			language.NA:      "Nauru",
			language.NAN:     "Cina Min Nan",
			language.NAP:     "Neapolitan",
			language.NAQ:     "Nama",
			language.NB:      "Bokmal Norway",
			language.ND:      "Ndebele Utara",
			language.NDS:     "Jerman Rendah",
			language.NDS_NL:  "Saxon Rendah",
			language.NE:      "Nepal",
			language.NEW:     "Newari",
			language.NG:      "Ndonga",
			language.NIA:     "Nias",
			language.NIU:     "Niu",
			language.NL:      "Belanda",
			language.NL_BE:   "Flemish",
			language.NMG:     "Kwasio",
			language.NN:      "Nynorsk Norway",
			language.NNH:     "Ngiemboon",
			language.NO:      "Norway",
			language.NOG:     "Nogai",
			language.NQO:     "N’ko",
			language.NR:      "Ndebele Selatan",
			language.NSO:     "Sotho Utara",
			language.NUS:     "Nuer",
			language.NV:      "Navajo",
			language.NY:      "Nyanja",
			language.NYN:     "Nyankole",
			language.OC:      "Occitania",
			language.OJB:     "Ojibwa Barat Laut",
			language.OJC:     "Ojibwa Tengah",
			language.OJS:     "Oji-Cree",
			language.OJW:     "Ojibwa Barat",
			language.OKA:     "Okanagan",
			language.OM:      "Oromo",
			language.OR:      "Odia",
			language.OS:      "Ossete",
			language.PA:      "Punjabi",
			language.PAG:     "Pangasinan",
			language.PAM:     "Pampanga",
			language.PAP:     "Papiamento",
			language.PAU:     "Palauan",
			language.PCM:     "Nigerian Pidgin",
			language.PIS:     "Pijin",
			language.PL:      "Poland",
			language.PQM:     "Maliseet-Passamaquoddy",
			language.PRG:     "Prusia",
			language.PS:      "Pushto",
			language.PT:      "Portugis",
			language.PT_BR:   "Portugis Brazil",
			language.PT_PT:   "Portugis Eropah",
			language.QU:      "Quechua",
			language.QUC:     "Kʼicheʼ",
			language.RAJ:     "Rajasthani",
			language.RAP:     "Rapanui",
			language.RAR:     "Rarotonga",
			language.RHG:     "Rohingya",
			language.RM:      "Romansh",
			language.RN:      "Rundi",
			language.RO:      "Romania",
			language.RO_MD:   "Moldavia",
			language.ROF:     "Rombo",
			language.RU:      "Rusia",
			language.RUP:     "Aromanian",
			language.RW:      "Kinyarwanda",
			language.RWK:     "Rwa",
			language.SA:      "Sanskrit",
			language.SAD:     "Sandawe",
			language.SAH:     "Sakha",
			language.SAQ:     "Samburu",
			language.SAT:     "Santali",
			language.SBA:     "Ngambay",
			language.SBP:     "Sangu",
			language.SC:      "Sardinia",
			language.SCN:     "Sicili",
			language.SCO:     "Scots",
			language.SD:      "Sindhi",
			language.SDH:     "Kurdish Selatan",
			language.SE:      "Sami Utara",
			language.SEE:     "Seneca",
			language.SEH:     "Sena",
			language.SES:     "Koyraboro Senni",
			language.SG:      "Sango",
			language.SH:      "SerboCroatia",
			language.SHI:     "Tachelhit",
			language.SHN:     "Shan",
			language.SHU:     "Arab Chadian",
			language.SI:      "Sinhala",
			language.SK:      "Slovak",
			language.SL:      "Slovenia",
			language.SLH:     "Lushootseed Selatan",
			language.SM:      "Samoa",
			language.SMA:     "Sami Selatan",
			language.SMJ:     "Lule Sami",
			language.SMN:     "Inari Sami",
			language.SMS:     "Skolt Sami",
			language.SN:      "Shona",
			language.SNK:     "Soninke",
			language.SO:      "Somali",
			language.SQ:      "Albania",
			language.SR:      "Serbia",
			language.SRN:     "Sranan Tongo",
			language.SS:      "Swati",
			language.SSY:     "Saho",
			language.ST:      "Sotho Selatan",
			language.STR:     "Straits Salish",
			language.SU:      "Sunda",
			language.SUK:     "Sukuma",
			language.SV:      "Sweden",
			language.SW:      "Swahili",
			language.SW_CD:   "Congo Swahili",
			language.SWB:     "Comoria",
			language.SYR:     "Syriac",
			language.SZL:     "Silesia",
			language.TA:      "Tamil",
			language.TCE:     "Tutchone Selatan",
			language.TE:      "Telugu",
			language.TEM:     "Timne",
			language.TEO:     "Teso",
			language.TET:     "Tetum",
			language.TG:      "Tajik",
			language.TGX:     "Tagish",
			language.TH:      "Thai",
			language.THT:     "Tahltan",
			language.TI:      "Tigrinya",
			language.TIG:     "Tigre",
			language.TK:      "Turkmen",
			language.TLH:     "Klingon",
			language.TLI:     "Tlingit",
			language.TLY:     "Talysh",
			language.TN:      "Tswana",
			language.TO:      "Tonga",
			language.TOK:     "Toki Pona",
			language.TPI:     "Tok Pisin",
			language.TR:      "Turki",
			language.TRV:     "Taroko",
			language.TS:      "Tsonga",
			language.TT:      "Tatar",
			language.TTM:     "Tutchone Utara",
			language.TUM:     "Tumbuka",
			language.TVL:     "Tuvalu",
			language.TW:      "Twi",
			language.TWQ:     "Tasawaq",
			language.TY:      "Tahiti",
			language.TYV:     "Tuvinian",
			language.TZM:     "Tamazight Atlas Tengah",
			language.UDM:     "Udmurt",
			language.UG:      "Uighur",
			language.UK:      "Ukraine",
			language.UMB:     "Umbundu",
			language.UND:     "Bahasa Tidak Diketahui",
			language.UR:      "Urdu",
			language.UZ:      "Uzbekistan",
			language.VAI:     "Vai",
			language.VE:      "Venda",
			language.VEC:     "Venetia",
			language.VI:      "Vietnam",
			language.VMW:     "Makhuwa",
			language.VO:      "Volapük",
			language.VUN:     "Vunjo",
			language.WA:      "Walloon",
			language.WAE:     "Walser",
			language.WAL:     "Wolaytta",
			language.WAR:     "Waray",
			language.WBP:     "Warlpiri",
			language.WO:      "Wolof",
			language.WUU:     "Cina Wu",
			language.XAL:     "Kalmyk",
			language.XH:      "Xhosa",
			language.XNR:     "Kangri",
			language.XOG:     "Soga",
			language.YAV:     "Yangben",
			language.YBB:     "Yemba",
			language.YI:      "Yiddish",
			language.YO:      "Yoruba",
			language.YRL:     "Nheengatu",
			language.YUE:     "Cina, Kantonis",
			language.ZA:      "Zhuang",
			language.ZGH:     "Tamazight Maghribi Standard",
			language.ZH:      "Cina, Mandarin",
			language.ZH_HANS: "Cina Mandarin Ringkas",
			language.ZH_HANT: "Cina Mandarin Tradisional",
			language.ZU:      "Zulu",
			language.ZUN:     "Zuni",
			language.ZXX:     "Tiada kandungan linguistik",
			language.ZZA:     "Zaza",
		},
		Territories: cldr.Territories{
			territory.V_001: "Dunia",
			territory.V_002: "Afrika",
			territory.V_003: "Amerika Utara",
			territory.V_005: "Amerika Selatan",
			territory.V_009: "Oceania",
			territory.V_011: "Afrika Barat",
			territory.V_013: "Amerika Tengah",
			territory.V_014: "Afrika Timur",
			territory.V_015: "Afrika Utara",
			territory.V_017: "Afrika Tengah",
			territory.V_018: "Selatan Afrika",
			territory.V_019: "Amerika",
			territory.V_021: "Utara Amerika",
			territory.V_029: "Caribbean",
			territory.V_030: "Asia Timur",
			territory.V_034: "Asia Selatan",
			territory.V_035: "Asia Tenggara",
			territory.V_039: "Eropah Selatan",
			territory.V_053: "Australasia",
			territory.V_054: "Melanesia",
			territory.V_057: "Wilayah Mikronesia",
			territory.V_061: "Polinesia",
			territory.V_142: "Asia",
			territory.V_143: "Asia Tengah",
			territory.V_145: "Asia Barat",
			territory.V_150: "Eropah",
			territory.V_151: "Eropah Timur",
			territory.V_154: "Eropah Utara",
			territory.V_155: "Eropah Barat",
			territory.V_202: "Afrika Sub-Sahara",
			territory.V_419: "Amerika Latin",
			territory.AC:    "Pulau Ascension",
			territory.AD:    "Andorra",
			territory.AE:    "Emiriah Arab Bersatu",
			territory.AF:    "Afghanistan",
			territory.AG:    "Antigua dan Barbuda",
			territory.AI:    "Anguilla",
			territory.AL:    "Albania",
			territory.AM:    "Armenia",
			territory.AO:    "Angola",
			territory.AQ:    "Antartika",
			territory.AR:    "Argentina",
			territory.AS:    "Samoa Amerika",
			territory.AT:    "Austria",
			territory.AU:    "Australia",
			territory.AW:    "Aruba",
			territory.AX:    "Kepulauan Aland",
			territory.AZ:    "Azerbaijan",
			territory.BA:    "Bosnia dan Herzegovina",
			territory.BB:    "Barbados",
			territory.BD:    "Bangladesh",
			territory.BE:    "Belgium",
			territory.BF:    "Burkina Faso",
			territory.BG:    "Bulgaria",
			territory.BH:    "Bahrain",
			territory.BI:    "Burundi",
			territory.BJ:    "Benin",
			territory.BL:    "St. Barthelemy",
			territory.BM:    "Bermuda",
			territory.BN:    "Brunei",
			territory.BO:    "Bolivia",
			territory.BQ:    "Belanda Caribbean",
			territory.BR:    "Brazil",
			territory.BS:    "Bahamas",
			territory.BT:    "Bhutan",
			territory.BV:    "Pulau Bouvet",
			territory.BW:    "Botswana",
			territory.BY:    "Belarus",
			territory.BZ:    "Belize",
			territory.CA:    "Kanada",
			territory.CC:    "Kepulauan Cocos (Keeling)",
			territory.CD:    "Congo - Kinshasa",
			territory.CF:    "Republik Afrika Tengah",
			territory.CG:    "Congo - Brazzaville",
			territory.CH:    "Switzerland",
			territory.CI:    "Cote d’Ivoire",
			territory.CK:    "Kepulauan Cook",
			territory.CL:    "Chile",
			territory.CM:    "Cameroon",
			territory.CN:    "China",
			territory.CO:    "Colombia",
			territory.CP:    "Pulau Clipperton",
			territory.CR:    "Costa Rica",
			territory.CU:    "Cuba",
			territory.CV:    "Cape Verde",
			territory.CW:    "Curacao",
			territory.CX:    "Pulau Krismas",
			territory.CY:    "Cyprus",
			territory.CZ:    "Czechia",
			territory.DE:    "Jerman",
			territory.DG:    "Diego Garcia",
			territory.DJ:    "Djibouti",
			territory.DK:    "Denmark",
			territory.DM:    "Dominica",
			territory.DO:    "Republik Dominica",
			territory.DZ:    "Algeria",
			territory.EA:    "Ceuta dan Melilla",
			territory.EC:    "Ecuador",
			territory.EE:    "Estonia",
			territory.EG:    "Mesir",
			territory.EH:    "Sahara Barat",
			territory.ER:    "Eritrea",
			territory.ES:    "Sepanyol",
			territory.ET:    "Ethiopia",
			territory.EU:    "Kesatuan Eropah",
			territory.EZ:    "Zon Euro",
			territory.FI:    "Finland",
			territory.FJ:    "Fiji",
			territory.FK:    "Kepulauan Falkland",
			territory.FM:    "Micronesia",
			territory.FO:    "Kepulauan Faroe",
			territory.FR:    "Perancis",
			territory.GA:    "Gabon",
			territory.GB:    "United Kingdom",
			territory.GD:    "Grenada",
			territory.GE:    "Georgia",
			territory.GF:    "Guiana Perancis",
			territory.GG:    "Guernsey",
			territory.GH:    "Ghana",
			territory.GI:    "Gibraltar",
			territory.GL:    "Greenland",
			territory.GM:    "Gambia",
			territory.GN:    "Guinea",
			territory.GP:    "Guadeloupe",
			territory.GQ:    "Guinea Khatulistiwa",
			territory.GR:    "Greece",
			territory.GS:    "Kepulauan Georgia Selatan & Sandwich Selatan",
			territory.GT:    "Guatemala",
			territory.GU:    "Guam",
			territory.GW:    "Guinea Bissau",
			territory.GY:    "Guyana",
			territory.HK:    "Hong Kong SAR China",
			territory.HM:    "Kepulauan Heard & McDonald",
			territory.HN:    "Honduras",
			territory.HR:    "Croatia",
			territory.HT:    "Haiti",
			territory.HU:    "Hungary",
			territory.IC:    "Kepulauan Canary",
			territory.ID:    "Indonesia",
			territory.IE:    "Ireland",
			territory.IL:    "Israel",
			territory.IM:    "Isle of Man",
			territory.IN:    "India",
			territory.IO:    "Wilayah Lautan Hindi British",
			territory.IQ:    "Iraq",
			territory.IR:    "Iran",
			territory.IS:    "Iceland",
			territory.IT:    "Itali",
			territory.JE:    "Jersey",
			territory.JM:    "Jamaica",
			territory.JO:    "Jordan",
			territory.JP:    "Jepun",
			territory.KE:    "Kenya",
			territory.KG:    "Kyrgyzstan",
			territory.KH:    "Kemboja",
			territory.KI:    "Kiribati",
			territory.KM:    "Comoros",
			territory.KN:    "Saint Kitts dan Nevis",
			territory.KP:    "Korea Utara",
			territory.KR:    "Korea Selatan",
			territory.KW:    "Kuwait",
			territory.KY:    "Kepulauan Cayman",
			territory.KZ:    "Kazakhstan",
			territory.LA:    "Laos",
			territory.LB:    "Lubnan",
			territory.LC:    "Saint Lucia",
			territory.LI:    "Liechtenstein",
			territory.LK:    "Sri Lanka",
			territory.LR:    "Liberia",
			territory.LS:    "Lesotho",
			territory.LT:    "Lithuania",
			territory.LU:    "Luxembourg",
			territory.LV:    "Latvia",
			territory.LY:    "Libya",
			territory.MA:    "Maghribi",
			territory.MC:    "Monaco",
			territory.MD:    "Moldova",
			territory.ME:    "Montenegro",
			territory.MF:    "Saint Martin",
			territory.MG:    "Madagaskar",
			territory.MH:    "Kepulauan Marshall",
			territory.MK:    "Macedonia Utara",
			territory.ML:    "Mali",
			territory.MM:    "Myanmar (Burma)",
			territory.MN:    "Mongolia",
			territory.MO:    "Macau SAR China",
			territory.MP:    "Kepulauan Mariana Utara",
			territory.MQ:    "Martinique",
			territory.MR:    "Mauritania",
			territory.MS:    "Montserrat",
			territory.MT:    "Malta",
			territory.MU:    "Mauritius",
			territory.MV:    "Maldives",
			territory.MW:    "Malawi",
			territory.MX:    "Mexico",
			territory.MY:    "Malaysia",
			territory.MZ:    "Mozambique",
			territory.NA:    "Namibia",
			territory.NC:    "New Caledonia",
			territory.NE:    "Niger",
			territory.NF:    "Pulau Norfolk",
			territory.NG:    "Nigeria",
			territory.NI:    "Nicaragua",
			territory.NL:    "Belanda",
			territory.NO:    "Norway",
			territory.NP:    "Nepal",
			territory.NR:    "Nauru",
			territory.NU:    "Niue",
			territory.NZ:    "New Zealand",
			territory.OM:    "Oman",
			territory.PA:    "Panama",
			territory.PE:    "Peru",
			territory.PF:    "Polinesia Perancis",
			territory.PG:    "Papua New Guinea",
			territory.PH:    "Filipina",
			territory.PK:    "Pakistan",
			territory.PL:    "Poland",
			territory.PM:    "Saint Pierre dan Miquelon",
			territory.PN:    "Kepulauan Pitcairn",
			territory.PR:    "Puerto Rico",
			territory.PS:    "Wilayah Palestin",
			territory.PT:    "Portugal",
			territory.PW:    "Palau",
			territory.PY:    "Paraguay",
			territory.QA:    "Qatar",
			territory.QO:    "Oceania Terpencil",
			territory.RE:    "Reunion",
			territory.RO:    "Romania",
			territory.RS:    "Serbia",
			territory.RU:    "Rusia",
			territory.RW:    "Rwanda",
			territory.SA:    "Arab Saudi",
			territory.SB:    "Kepulauan Solomon",
			territory.SC:    "Seychelles",
			territory.SD:    "Sudan",
			territory.SE:    "Sweden",
			territory.SG:    "Singapura",
			territory.SH:    "Saint Helena",
			territory.SI:    "Slovenia",
			territory.SJ:    "Svalbard dan Jan Mayen",
			territory.SK:    "Slovakia",
			territory.SL:    "Sierra Leone",
			territory.SM:    "San Marino",
			territory.SN:    "Senegal",
			territory.SO:    "Somalia",
			territory.SR:    "Surinam",
			territory.SS:    "Sudan Selatan",
			territory.ST:    "Sao Tome dan Principe",
			territory.SV:    "El Salvador",
			territory.SX:    "Sint Maarten",
			territory.SY:    "Syria",
			territory.SZ:    "Eswatini",
			territory.TA:    "Tristan da Cunha",
			territory.TC:    "Kepulauan Turks dan Caicos",
			territory.TD:    "Chad",
			territory.TF:    "Wilayah Selatan Perancis",
			territory.TG:    "Togo",
			territory.TH:    "Thailand",
			territory.TJ:    "Tajikistan",
			territory.TK:    "Tokelau",
			territory.TL:    "Timor-Leste",
			territory.TM:    "Turkmenistan",
			territory.TN:    "Tunisia",
			territory.TO:    "Tonga",
			territory.TR:    "Turkiye",
			territory.TT:    "Trinidad dan Tobago",
			territory.TV:    "Tuvalu",
			territory.TW:    "Taiwan",
			territory.TZ:    "Tanzania",
			territory.UA:    "Ukraine",
			territory.UG:    "Uganda",
			territory.UM:    "Kepulauan Terpencil A.S.",
			territory.UN:    "Bangsa-bangsa Bersatu",
			territory.US:    "Amerika Syarikat",
			territory.UY:    "Uruguay",
			territory.UZ:    "Uzbekistan",
			territory.VA:    "Kota Vatican",
			territory.VC:    "Saint Vincent dan Grenadines",
			territory.VE:    "Venezuela",
			territory.VG:    "Kepulauan Virgin British",
			territory.VI:    "Kepulauan Virgin A.S.",
			territory.VN:    "Vietnam",
			territory.VU:    "Vanuatu",
			territory.WF:    "Wallis dan Futuna",
			territory.WS:    "Samoa",
			territory.XA:    "Aksen Pseudo",
			territory.XB:    "Bidi Pseudo",
			territory.XK:    "Kosovo",
			territory.YE:    "Yaman",
			territory.YT:    "Mayotte",
			territory.ZA:    "Afrika Selatan",
			territory.ZM:    "Zambia",
			territory.ZW:    "Zimbabwe",
			territory.ZZ:    "Wilayah Tidak Diketahui",
		},
	}
}

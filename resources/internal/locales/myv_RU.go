// Code generated by make_resources.go; DO NOT EDIT.
package locales

import (
	"github.com/razor-1/cldr"
	"github.com/razor-1/cldr/resources/currency"
	"github.com/razor-1/cldr/resources/language"
	"github.com/razor-1/cldr/resources/territory"
)

func Get_myv_RU() *cldr.Locale {
	return &cldr.Locale{
		Locale:   "myv_RU",
		Calendar: cldr.Calendar{Formats: cldr.CalendarFormats{Date: cldr.CalendarDateFormat{Full: "y MMMM d, EEEE", Long: "y MMMM d", Medium: "y MMM d", Short: "y-MM-dd"}, Time: cldr.CalendarDateFormat{Full: "HH:mm:ss zzzz", Long: "HH:mm:ss z", Medium: "HH:mm:ss", Short: "HH:mm"}, DateTime: cldr.CalendarDateFormat{Full: "{1} {0}", Long: "{1} {0}", Medium: "{1} {0}", Short: "{1} {0}"}, GMT: "GMT{0}"}, FormatNames: cldr.CalendarFormatNames{Months: cldr.CalendarMonthFormatNames{Abbreviated: cldr.CalendarMonthFormatNameValue{Jan: "якш", Feb: "дав", Mar: "эйз", Apr: "чад", May: "пан", Jun: "ашт", Jul: "мед", Aug: "ума", Sep: "таш", Oct: "ожо", Nov: "сун", Dec: "аца"}, Narrow: cldr.CalendarMonthFormatNameValue{Jan: "1", Feb: "2", Mar: "3", Apr: "4", May: "5", Jun: "6", Jul: "7", Aug: "8", Sep: "9", Oct: "10", Nov: "11", Dec: "12"}, Short: cldr.CalendarMonthFormatNameValue{Jan: "", Feb: "", Mar: "", Apr: "", May: "", Jun: "", Jul: "", Aug: "", Sep: "", Oct: "", Nov: "", Dec: ""}, Wide: cldr.CalendarMonthFormatNameValue{Jan: "якшамков", Feb: "даволков", Mar: "эйзюрков", Apr: "чадыков", May: "панжиков", Jun: "аштемков", Jul: "медьков", Aug: "умарьков", Sep: "таштамков", Oct: "ожоков", Nov: "сундерьков", Dec: "ацамков"}}, Days: cldr.CalendarDayFormatNames{Abbreviated: cldr.CalendarDayFormatNameValue{Sun: "тар", Mon: "атя", Tue: "вас", Wed: "кун", Thu: "кал", Fri: "сюк", Sat: "шля"}, Narrow: cldr.CalendarDayFormatNameValue{Sun: "S", Mon: "M", Tue: "T", Wed: "W", Thu: "T", Fri: "F", Sat: "S"}, Short: cldr.CalendarDayFormatNameValue{Sun: "", Mon: "", Tue: "", Wed: "", Thu: "", Fri: "", Sat: ""}, Wide: cldr.CalendarDayFormatNameValue{Sun: "таргочи", Mon: "атяньчи", Tue: "вастаньчи", Wed: "куншкачи", Thu: "калоньчи", Fri: "сюконьчи", Sat: "шлямочи"}}, Periods: cldr.CalendarPeriodFormatNames{Abbreviated: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Narrow: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Short: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Wide: cldr.CalendarPeriodFormatNameValue{AM: "обедтэ икеле", PM: "обедтэ мейле"}}}},
		Number: cldr.Number{
			Symbols: cldr.Symbols{Decimal: ".", Group: ",", Negative: "-", Percent: "%", PerMille: "‰"},
			Formats: cldr.NumberFormats{Decimal: "#,##0.###", Currency: "", CurrencyAccounting: "", Percent: ""},
			Currencies: cldr.Currencies{
				currency.AFN: cldr.Currency{DisplayName: "", Symbol: "؋"},
				currency.AMD: cldr.Currency{DisplayName: "", Symbol: "֏"},
				currency.AOA: cldr.Currency{DisplayName: "", Symbol: "Kz"},
				currency.ARS: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.AUD: cldr.Currency{DisplayName: "", Symbol: "A$"},
				currency.AZN: cldr.Currency{DisplayName: "", Symbol: "₼"},
				currency.BAM: cldr.Currency{DisplayName: "", Symbol: "KM"},
				currency.BBD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.BDT: cldr.Currency{DisplayName: "", Symbol: "৳"},
				currency.BMD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.BND: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.BOB: cldr.Currency{DisplayName: "", Symbol: "Bs"},
				currency.BRL: cldr.Currency{DisplayName: "", Symbol: "R$"},
				currency.BSD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.BWP: cldr.Currency{DisplayName: "", Symbol: "P"},
				currency.BZD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.CAD: cldr.Currency{DisplayName: "", Symbol: "CA$"},
				currency.CLP: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.CNY: cldr.Currency{DisplayName: "", Symbol: "CN¥"},
				currency.COP: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.CRC: cldr.Currency{DisplayName: "", Symbol: "₡"},
				currency.CUC: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.CUP: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.CZK: cldr.Currency{DisplayName: "", Symbol: "Kč"},
				currency.DKK: cldr.Currency{DisplayName: "", Symbol: "kr"},
				currency.DOP: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.EGP: cldr.Currency{DisplayName: "", Symbol: "E£"},
				currency.ESP: cldr.Currency{DisplayName: "", Symbol: "₧"},
				currency.EUR: cldr.Currency{DisplayName: "", Symbol: "€"},
				currency.FJD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.FKP: cldr.Currency{DisplayName: "", Symbol: "£"},
				currency.GBP: cldr.Currency{DisplayName: "", Symbol: "£"},
				currency.GEL: cldr.Currency{DisplayName: "", Symbol: "₾"},
				currency.GHS: cldr.Currency{DisplayName: "", Symbol: "GH₵"},
				currency.GIP: cldr.Currency{DisplayName: "", Symbol: "£"},
				currency.GNF: cldr.Currency{DisplayName: "", Symbol: "FG"},
				currency.GTQ: cldr.Currency{DisplayName: "", Symbol: "Q"},
				currency.GYD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.HKD: cldr.Currency{DisplayName: "", Symbol: "HK$"},
				currency.HNL: cldr.Currency{DisplayName: "", Symbol: "L"},
				currency.HRK: cldr.Currency{DisplayName: "", Symbol: "kn"},
				currency.HUF: cldr.Currency{DisplayName: "", Symbol: "Ft"},
				currency.IDR: cldr.Currency{DisplayName: "", Symbol: "Rp"},
				currency.ILS: cldr.Currency{DisplayName: "", Symbol: "₪"},
				currency.INR: cldr.Currency{DisplayName: "", Symbol: "₹"},
				currency.ISK: cldr.Currency{DisplayName: "", Symbol: "kr"},
				currency.JMD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.JPY: cldr.Currency{DisplayName: "", Symbol: "JP¥"},
				currency.KGS: cldr.Currency{DisplayName: "", Symbol: "⃀"},
				currency.KHR: cldr.Currency{DisplayName: "", Symbol: "៛"},
				currency.KMF: cldr.Currency{DisplayName: "", Symbol: "CF"},
				currency.KPW: cldr.Currency{DisplayName: "", Symbol: "₩"},
				currency.KRW: cldr.Currency{DisplayName: "", Symbol: "₩"},
				currency.KYD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.KZT: cldr.Currency{DisplayName: "", Symbol: "₸"},
				currency.LAK: cldr.Currency{DisplayName: "", Symbol: "₭"},
				currency.LBP: cldr.Currency{DisplayName: "", Symbol: "L£"},
				currency.LKR: cldr.Currency{DisplayName: "", Symbol: "Rs"},
				currency.LRD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.LTL: cldr.Currency{DisplayName: "", Symbol: "Lt"},
				currency.LVL: cldr.Currency{DisplayName: "", Symbol: "Ls"},
				currency.MGA: cldr.Currency{DisplayName: "", Symbol: "Ar"},
				currency.MMK: cldr.Currency{DisplayName: "", Symbol: "K"},
				currency.MNT: cldr.Currency{DisplayName: "", Symbol: "₮"},
				currency.MUR: cldr.Currency{DisplayName: "", Symbol: "Rs"},
				currency.MXN: cldr.Currency{DisplayName: "", Symbol: "MX$"},
				currency.MYR: cldr.Currency{DisplayName: "", Symbol: "RM"},
				currency.NAD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.NGN: cldr.Currency{DisplayName: "", Symbol: "₦"},
				currency.NIO: cldr.Currency{DisplayName: "", Symbol: "C$"},
				currency.NOK: cldr.Currency{DisplayName: "", Symbol: "kr"},
				currency.NPR: cldr.Currency{DisplayName: "", Symbol: "Rs"},
				currency.NZD: cldr.Currency{DisplayName: "", Symbol: "NZ$"},
				currency.PHP: cldr.Currency{DisplayName: "", Symbol: "₱"},
				currency.PKR: cldr.Currency{DisplayName: "", Symbol: "Rs"},
				currency.PLN: cldr.Currency{DisplayName: "", Symbol: "zł"},
				currency.PYG: cldr.Currency{DisplayName: "", Symbol: "₲"},
				currency.RON: cldr.Currency{DisplayName: "", Symbol: "lei"},
				currency.RUB: cldr.Currency{DisplayName: "", Symbol: "₽"},
				currency.RWF: cldr.Currency{DisplayName: "", Symbol: "RF"},
				currency.SBD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.SEK: cldr.Currency{DisplayName: "", Symbol: "kr"},
				currency.SGD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.SHP: cldr.Currency{DisplayName: "", Symbol: "£"},
				currency.SRD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.SSP: cldr.Currency{DisplayName: "", Symbol: "£"},
				currency.STN: cldr.Currency{DisplayName: "", Symbol: "Db"},
				currency.SYP: cldr.Currency{DisplayName: "", Symbol: "£"},
				currency.THB: cldr.Currency{DisplayName: "", Symbol: "฿"},
				currency.TOP: cldr.Currency{DisplayName: "", Symbol: "T$"},
				currency.TRY: cldr.Currency{DisplayName: "", Symbol: "₺"},
				currency.TTD: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.TWD: cldr.Currency{DisplayName: "", Symbol: "NT$"},
				currency.UAH: cldr.Currency{DisplayName: "", Symbol: "₴"},
				currency.USD: cldr.Currency{DisplayName: "", Symbol: "US$"},
				currency.UYU: cldr.Currency{DisplayName: "", Symbol: "$"},
				currency.VEF: cldr.Currency{DisplayName: "", Symbol: "Bs"},
				currency.VND: cldr.Currency{DisplayName: "", Symbol: "₫"},
				currency.XAF: cldr.Currency{DisplayName: "", Symbol: "FCFA"},
				currency.XCD: cldr.Currency{DisplayName: "", Symbol: "EC$"},
				currency.XCG: cldr.Currency{DisplayName: "", Symbol: "Cg."},
				currency.XOF: cldr.Currency{DisplayName: "", Symbol: "F\u202fCFA"},
				currency.XPF: cldr.Currency{DisplayName: "", Symbol: "CFPF"},
				currency.XXX: cldr.Currency{DisplayName: "", Symbol: "¤"},
				currency.ZAR: cldr.Currency{DisplayName: "", Symbol: "R"},
				currency.ZMW: cldr.Currency{DisplayName: "", Symbol: "ZK"},
			},
		},
		Display: cldr.LocaleDisplayPattern{Pattern: "{0} ({1})", Separator: "{0}, {1}", KeyTypePattern: "{0}: {1}"},
		Languages: cldr.Languages{
			language.AB:    "абхазонь кель",
			language.ACE:   "ачехень кель",
			language.DE:    "немецень кель",
			language.DE_AT: "Австриянь немецень кель",
			language.DE_CH: "Швецариянь немецень кель",
			language.EN:    "англонь кель",
			language.EN_AU: "Австралиянь англонь кель",
			language.EN_CA: "Канадань англонь кель",
			language.EN_GB: "Британиянь англонь кель",
			language.EN_US: "Американь англонь кель",
			language.ES:    "испанонь кель",
			language.ES_ES: "Европань испанонь кель",
			language.FR:    "французонь кель",
			language.FR_CA: "Канадань французонь кель",
			language.FR_CH: "Швецариянь французонь кель",
			language.IT:    "италиянь кель",
			language.JA:    "япононь кель",
			language.KO:    "Кореань кель",
			language.MYV:   "эрзянь кель",
			language.PL:    "польшань кель",
			language.PT:    "португалонь кель",
			language.RU:    "рузонь кель",
			language.UND:   "асодавикс кель",
			language.ZH:    "китаень кель",
		},
		Territories: cldr.Territories{
			territory.V_001: "Масторланго",
			territory.V_002: "Африка",
			territory.V_011: "Африкань чивалгома ёнкс",
			territory.V_014: "Африкань чилисема ёнкс",
			territory.V_015: "Африкань пелеве ёнкс",
			territory.V_017: "Африкань куншка",
			territory.V_019: "Америкат",
			territory.V_030: "Азиянь чилисема ёнкс",
			territory.V_034: "Азиянь чинеле ёнкс",
			territory.V_035: "Азиянь чинеле-чилисема ёнкс",
			territory.V_039: "Европань чипеле ёнкс",
			territory.V_142: "Азия",
			territory.V_143: "Азиянь куншка",
			territory.V_145: "Азиянь чивалгома ёнкс",
			territory.V_150: "Европа",
			territory.V_151: "Европань чилисема ёнкс",
			territory.V_154: "Европань пелеве ёнкс",
			territory.V_155: "Европань чивалгома ёнкс",
			territory.AD:    "Андорра",
			territory.AF:    "Афганистан",
			territory.AL:    "Албания",
			territory.AM:    "Арменэнь мастор",
			territory.AO:    "Ангола",
			territory.AQ:    "Антарктида",
			territory.AR:    "Аргентина",
			territory.AS:    "Американь Самоа",
			territory.AT:    "Австрия",
			territory.AU:    "Австралия",
			territory.AX:    "Аландонь усият",
			territory.BB:    "Барбадос",
			territory.BD:    "Бангладеш",
			territory.BE:    "Белгия",
			territory.BG:    "Болгария",
			territory.BI:    "Бурунди",
			territory.BJ:    "Бенин",
			territory.BM:    "Бермуда",
			territory.BO:    "Боливия",
			territory.BR:    "Бразил",
			territory.BW:    "Ботсвана",
			territory.BY:    "Беларусия",
			territory.CA:    "Канада",
			territory.CH:    "Швейцария",
			territory.CK:    "Кук усият",
			territory.CL:    "Чили",
			territory.CN:    "Китай",
			territory.CO:    "Колумбия",
			territory.CU:    "Куба",
			territory.CZ:    "Чехия",
			territory.DE:    "Германия",
			territory.DK:    "Дания",
			territory.DZ:    "Алгерия",
			territory.EE:    "Эстэнь мастор",
			territory.ER:    "Эритрея",
			territory.ES:    "Испания",
			territory.FI:    "Финнэнь мастор",
			territory.FJ:    "Фиджи",
			territory.FO:    "Фарерэнь усият",
			territory.FR:    "Франция",
			territory.GA:    "Габон",
			territory.GD:    "Гренада",
			territory.GM:    "Гамбия",
			territory.GR:    "Грекень мастор",
			territory.GT:    "Гватемала",
			territory.GU:    "Гуам",
			territory.HR:    "Хорватия",
			territory.HT:    "Гаити",
			territory.HU:    "Венгрия",
			territory.IE:    "Ирландия",
			territory.IM:    "Ман усия",
			territory.IN:    "Индия",
			territory.IR:    "Иран",
			territory.IS:    "Исландия",
			territory.IT:    "Италия",
			territory.JP:    "Япононь мастор",
			territory.LI:    "Лихтенштейн",
			territory.LT:    "Литва",
			territory.LU:    "Люксембург",
			territory.LV:    "Латвия",
			territory.MC:    "Монако",
			territory.MD:    "Молдова",
			territory.ME:    "Монтенегро",
			territory.ML:    "Мали",
			territory.MN:    "Монголонь мастор",
			territory.MT:    "Малта",
			territory.MW:    "Малави",
			territory.NA:    "Намибия",
			territory.NC:    "Од Каледония",
			territory.NE:    "Нигер",
			territory.NG:    "Нигерия",
			territory.NL:    "Нидерланд",
			territory.NO:    "Норвегия",
			territory.NP:    "Непал",
			territory.NR:    "Науру",
			territory.NZ:    "Од Зеландия",
			territory.PA:    "Панама",
			territory.PE:    "Перу",
			territory.PK:    "Пакистан",
			territory.PL:    "Польша",
			territory.PT:    "Португалонь мастор",
			territory.PY:    "Парагвай",
			territory.RO:    "Румыния",
			territory.RS:    "Сербень мастор",
			territory.RU:    "Рузонь мастор",
			territory.SB:    "Соломон усият",
			territory.SD:    "Судан",
			territory.SE:    "Шведэнь мастор",
			territory.SI:    "Словения",
			territory.SK:    "Словакия",
			territory.SN:    "Сенегал",
			territory.SO:    "Сомалия",
			territory.TD:    "Чад",
			territory.TG:    "Того",
			territory.TH:    "Таймастор",
			territory.TO:    "Тонга",
			territory.TV:    "Тувалу",
			territory.TW:    "Тайван",
			territory.TZ:    "Танзания",
			territory.UA:    "Украина",
			territory.UG:    "Уганда",
			territory.UN:    "Вейсэндязь Раськетнень Организация",
			territory.US:    "Американь Вейсэндявкс Штаттнэ",
			territory.UY:    "Уругвай",
			territory.VA:    "Ватикан ош",
			territory.VU:    "Вануату",
			territory.WS:    "Самоа",
			territory.XK:    "Косово",
			territory.ZM:    "Замбия",
			territory.ZW:    "Зимбабве",
			territory.ZZ:    "Асодавикс Ёнкс",
		},
	}
}

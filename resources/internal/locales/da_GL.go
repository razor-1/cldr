// Code generated by make_resources.go; DO NOT EDIT.
package locales

import (
	"github.com/razor-1/cldr"
	"github.com/razor-1/cldr/resources/currency"
	"github.com/razor-1/cldr/resources/language"
	"github.com/razor-1/cldr/resources/territory"
)

func Get_da_GL() *cldr.Locale {
	return &cldr.Locale{
		Locale:   "da_GL",
		Calendar: cldr.Calendar{Formats: cldr.CalendarFormats{Date: cldr.CalendarDateFormat{Full: "EEEE 'den' d. MMMM y", Long: "d. MMMM y", Medium: "d. MMM y", Short: "dd.MM.y"}, Time: cldr.CalendarDateFormat{Full: "HH.mm.ss zzzz", Long: "HH.mm.ss z", Medium: "HH.mm.ss", Short: "HH.mm"}, DateTime: cldr.CalendarDateFormat{Full: "{1} {0}", Long: "{1} {0}", Medium: "{1} {0}", Short: "{1} {0}"}, GMT: "GMT{0}"}, FormatNames: cldr.CalendarFormatNames{Months: cldr.CalendarMonthFormatNames{Abbreviated: cldr.CalendarMonthFormatNameValue{Jan: "jan.", Feb: "feb.", Mar: "mar.", Apr: "apr.", May: "maj", Jun: "jun.", Jul: "jul.", Aug: "aug.", Sep: "sep.", Oct: "okt.", Nov: "nov.", Dec: "dec."}, Narrow: cldr.CalendarMonthFormatNameValue{Jan: "J", Feb: "F", Mar: "M", Apr: "A", May: "M", Jun: "J", Jul: "J", Aug: "A", Sep: "S", Oct: "O", Nov: "N", Dec: "D"}, Short: cldr.CalendarMonthFormatNameValue{Jan: "", Feb: "", Mar: "", Apr: "", May: "", Jun: "", Jul: "", Aug: "", Sep: "", Oct: "", Nov: "", Dec: ""}, Wide: cldr.CalendarMonthFormatNameValue{Jan: "januar", Feb: "februar", Mar: "marts", Apr: "april", May: "maj", Jun: "juni", Jul: "juli", Aug: "august", Sep: "september", Oct: "oktober", Nov: "november", Dec: "december"}}, Days: cldr.CalendarDayFormatNames{Abbreviated: cldr.CalendarDayFormatNameValue{Sun: "søn.", Mon: "man.", Tue: "tirs.", Wed: "ons.", Thu: "tors.", Fri: "fre.", Sat: "lør."}, Narrow: cldr.CalendarDayFormatNameValue{Sun: "S", Mon: "M", Tue: "T", Wed: "O", Thu: "T", Fri: "F", Sat: "L"}, Short: cldr.CalendarDayFormatNameValue{Sun: "sø.", Mon: "ma.", Tue: "ti.", Wed: "on.", Thu: "to.", Fri: "fr.", Sat: "lø."}, Wide: cldr.CalendarDayFormatNameValue{Sun: "søndag", Mon: "mandag", Tue: "tirsdag", Wed: "onsdag", Thu: "torsdag", Fri: "fredag", Sat: "lørdag"}}, Periods: cldr.CalendarPeriodFormatNames{Abbreviated: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Narrow: cldr.CalendarPeriodFormatNameValue{AM: "a", PM: "p"}, Short: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}, Wide: cldr.CalendarPeriodFormatNameValue{AM: "", PM: ""}}}},
		Number: cldr.Number{
			Symbols: cldr.Symbols{Decimal: ",", Group: ".", Negative: "-", Percent: "%", PerMille: "‰"},
			Formats: cldr.NumberFormats{Decimal: "#,##0.###", Currency: "#,##0.00\u00a0¤", CurrencyAccounting: "", Percent: "#,##0\u00a0%"},
			Currencies: cldr.Currencies{
				currency.ADP: cldr.Currency{DisplayName: "Andorransk peseta", Symbol: ""},
				currency.AED: cldr.Currency{DisplayName: "dirham fra de Forenede Arabiske Emirater", Symbol: ""},
				currency.AFA: cldr.Currency{DisplayName: "Afghansk afghani (1927–2002)", Symbol: ""},
				currency.AFN: cldr.Currency{DisplayName: "afghansk afghani", Symbol: "؋"},
				currency.ALK: cldr.Currency{DisplayName: "albansk lek (1946–1965)", Symbol: ""},
				currency.ALL: cldr.Currency{DisplayName: "albansk lek", Symbol: ""},
				currency.AMD: cldr.Currency{DisplayName: "armensk dram", Symbol: "֏"},
				currency.ANG: cldr.Currency{DisplayName: "Nederlandske Antiller-gylden", Symbol: ""},
				currency.AOA: cldr.Currency{DisplayName: "angolansk kwanza", Symbol: "Kz"},
				currency.AOK: cldr.Currency{DisplayName: "Angolansk kwanza (1977–1990)", Symbol: ""},
				currency.AON: cldr.Currency{DisplayName: "Angolansk nye kwanza (1990–2000)", Symbol: ""},
				currency.AOR: cldr.Currency{DisplayName: "Angolansk kwanza (1995–1999)", Symbol: ""},
				currency.ARA: cldr.Currency{DisplayName: "Argentinsk austral", Symbol: ""},
				currency.ARL: cldr.Currency{DisplayName: "argentinsk peso ley (1970–1983)", Symbol: ""},
				currency.ARM: cldr.Currency{DisplayName: "argentinsk peso (1881–1970)", Symbol: ""},
				currency.ARP: cldr.Currency{DisplayName: "Argentinsk peso (1983–1985)", Symbol: ""},
				currency.ARS: cldr.Currency{DisplayName: "argentinsk peso", Symbol: "$"},
				currency.ATS: cldr.Currency{DisplayName: "Østrigsk schilling", Symbol: ""},
				currency.AUD: cldr.Currency{DisplayName: "australsk dollar", Symbol: "AU$"},
				currency.AWG: cldr.Currency{DisplayName: "arubansk florin", Symbol: ""},
				currency.AZM: cldr.Currency{DisplayName: "Aserbajdsjansk manat (1993–2006)", Symbol: ""},
				currency.AZN: cldr.Currency{DisplayName: "aserbajdsjansk manat", Symbol: "₼"},
				currency.BAD: cldr.Currency{DisplayName: "Bosnien-Hercegovinsk dinar", Symbol: ""},
				currency.BAM: cldr.Currency{DisplayName: "bosnien-hercegovinsk konvertibel mark", Symbol: "KM"},
				currency.BAN: cldr.Currency{DisplayName: "ny bosnien-hercegovinsk dinar (1994–1997)", Symbol: ""},
				currency.BBD: cldr.Currency{DisplayName: "barbadisk dollar", Symbol: "$"},
				currency.BDT: cldr.Currency{DisplayName: "bangladeshisk taka", Symbol: "৳"},
				currency.BEC: cldr.Currency{DisplayName: "Belgisk franc (konvertibel)", Symbol: ""},
				currency.BEF: cldr.Currency{DisplayName: "Belgisk franc", Symbol: ""},
				currency.BEL: cldr.Currency{DisplayName: "Belgisk franc (financial)", Symbol: ""},
				currency.BGL: cldr.Currency{DisplayName: "Bulgarsk hard lev", Symbol: ""},
				currency.BGM: cldr.Currency{DisplayName: "bulgarsk socialistisk lev", Symbol: ""},
				currency.BGN: cldr.Currency{DisplayName: "bulgarsk lev", Symbol: ""},
				currency.BGO: cldr.Currency{DisplayName: "bulgarsk lev (1879–1952)", Symbol: ""},
				currency.BHD: cldr.Currency{DisplayName: "bahrainsk dinar", Symbol: ""},
				currency.BIF: cldr.Currency{DisplayName: "burundisk franc", Symbol: ""},
				currency.BMD: cldr.Currency{DisplayName: "bermudansk dollar", Symbol: "$"},
				currency.BND: cldr.Currency{DisplayName: "bruneisk dollar", Symbol: "$"},
				currency.BOB: cldr.Currency{DisplayName: "boliviansk boliviano", Symbol: "Bs"},
				currency.BOL: cldr.Currency{DisplayName: "boliviansk boliviano (1863–1963)", Symbol: ""},
				currency.BOP: cldr.Currency{DisplayName: "Boliviansk peso", Symbol: ""},
				currency.BOV: cldr.Currency{DisplayName: "Boliviansk mvdol", Symbol: ""},
				currency.BRB: cldr.Currency{DisplayName: "Brasiliansk cruzeiro novo (1967–1986)", Symbol: ""},
				currency.BRC: cldr.Currency{DisplayName: "Brasiliansk cruzado (1986–1989)", Symbol: ""},
				currency.BRE: cldr.Currency{DisplayName: "Brasiliansk cruzeiro (1990–1993)", Symbol: ""},
				currency.BRL: cldr.Currency{DisplayName: "brasiliansk real", Symbol: "R$"},
				currency.BRN: cldr.Currency{DisplayName: "Brasiliansk cruzado novo (1989–1990)", Symbol: ""},
				currency.BRR: cldr.Currency{DisplayName: "Brasiliansk cruzeiro (1993–1994)", Symbol: ""},
				currency.BRZ: cldr.Currency{DisplayName: "brasiliansk cruzeiro (1942–1967)", Symbol: ""},
				currency.BSD: cldr.Currency{DisplayName: "bahamansk dollar", Symbol: "$"},
				currency.BTN: cldr.Currency{DisplayName: "bhutansk ngultrum", Symbol: ""},
				currency.BUK: cldr.Currency{DisplayName: "Burmesisk kyat", Symbol: ""},
				currency.BWP: cldr.Currency{DisplayName: "botswansk pula", Symbol: "P"},
				currency.BYB: cldr.Currency{DisplayName: "Hviderussisk rubel (1994–1999)", Symbol: ""},
				currency.BYN: cldr.Currency{DisplayName: "hviderussisk rubel", Symbol: "Br."},
				currency.BYR: cldr.Currency{DisplayName: "hviderussisk rubel (2000–2016)", Symbol: ""},
				currency.BZD: cldr.Currency{DisplayName: "belizisk dollar", Symbol: "$"},
				currency.CAD: cldr.Currency{DisplayName: "canadisk dollar", Symbol: "CA$"},
				currency.CDF: cldr.Currency{DisplayName: "congolesisk franc", Symbol: ""},
				currency.CHE: cldr.Currency{DisplayName: "WIR euro", Symbol: ""},
				currency.CHF: cldr.Currency{DisplayName: "schweizerfranc", Symbol: ""},
				currency.CHW: cldr.Currency{DisplayName: "WIR franc", Symbol: ""},
				currency.CLE: cldr.Currency{DisplayName: "chilensk escudo", Symbol: ""},
				currency.CLP: cldr.Currency{DisplayName: "chilensk peso", Symbol: "$"},
				currency.CNH: cldr.Currency{DisplayName: "kinesisk yuan (offshore)", Symbol: ""},
				currency.CNY: cldr.Currency{DisplayName: "kinesisk yuan", Symbol: "CN¥"},
				currency.COP: cldr.Currency{DisplayName: "colombiansk peso", Symbol: "$"},
				currency.CRC: cldr.Currency{DisplayName: "costaricansk colón", Symbol: "₡"},
				currency.CSD: cldr.Currency{DisplayName: "Serbisk dinar (2002–2006)", Symbol: ""},
				currency.CSK: cldr.Currency{DisplayName: "Tjekkoslovakisk hard koruna", Symbol: ""},
				currency.CUC: cldr.Currency{DisplayName: "cubansk konvertibel peso", Symbol: "$"},
				currency.CUP: cldr.Currency{DisplayName: "cubansk peso", Symbol: "$"},
				currency.CVE: cldr.Currency{DisplayName: "kapverdisk escudo", Symbol: ""},
				currency.CYP: cldr.Currency{DisplayName: "Cypriotisk pund", Symbol: ""},
				currency.CZK: cldr.Currency{DisplayName: "tjekkisk koruna", Symbol: "Kč"},
				currency.DDM: cldr.Currency{DisplayName: "Østtysk mark", Symbol: ""},
				currency.DEM: cldr.Currency{DisplayName: "Tysk mark", Symbol: ""},
				currency.DJF: cldr.Currency{DisplayName: "djiboutisk franc", Symbol: ""},
				currency.DKK: cldr.Currency{DisplayName: "dansk krone", Symbol: "kr."},
				currency.DOP: cldr.Currency{DisplayName: "dominikansk peso", Symbol: "$"},
				currency.DZD: cldr.Currency{DisplayName: "algerisk dinar", Symbol: ""},
				currency.ECS: cldr.Currency{DisplayName: "Ecuadoriansk sucre", Symbol: ""},
				currency.EEK: cldr.Currency{DisplayName: "Estisk kroon", Symbol: ""},
				currency.EGP: cldr.Currency{DisplayName: "egyptisk pund", Symbol: "E£"},
				currency.ERN: cldr.Currency{DisplayName: "eritreisk nakfa", Symbol: ""},
				currency.ESA: cldr.Currency{DisplayName: "Spansk peseta (A–konto)", Symbol: ""},
				currency.ESB: cldr.Currency{DisplayName: "Spansk peseta (konvertibel konto)", Symbol: ""},
				currency.ESP: cldr.Currency{DisplayName: "Spansk peseta", Symbol: "₧"},
				currency.ETB: cldr.Currency{DisplayName: "etiopisk birr", Symbol: ""},
				currency.EUR: cldr.Currency{DisplayName: "euro", Symbol: "€"},
				currency.FIM: cldr.Currency{DisplayName: "Finsk mark", Symbol: ""},
				currency.FJD: cldr.Currency{DisplayName: "fijiansk dollar", Symbol: "$"},
				currency.FKP: cldr.Currency{DisplayName: "pund fra Falklandsøerne", Symbol: "£"},
				currency.FRF: cldr.Currency{DisplayName: "Fransk franc", Symbol: ""},
				currency.GBP: cldr.Currency{DisplayName: "britisk pund", Symbol: "£"},
				currency.GEK: cldr.Currency{DisplayName: "Georgisk kupon larit", Symbol: ""},
				currency.GEL: cldr.Currency{DisplayName: "georgisk lari", Symbol: "₾"},
				currency.GHC: cldr.Currency{DisplayName: "Ghanesisk cedi (1979–2007)", Symbol: ""},
				currency.GHS: cldr.Currency{DisplayName: "ghanesisk cedi", Symbol: "GH₵"},
				currency.GIP: cldr.Currency{DisplayName: "gibraltarisk pund", Symbol: "£"},
				currency.GMD: cldr.Currency{DisplayName: "gambisk dalasi", Symbol: ""},
				currency.GNF: cldr.Currency{DisplayName: "guineansk franc", Symbol: "FG"},
				currency.GNS: cldr.Currency{DisplayName: "Guineansk syli", Symbol: ""},
				currency.GQE: cldr.Currency{DisplayName: "Ækvatorialguineask ekwele", Symbol: ""},
				currency.GRD: cldr.Currency{DisplayName: "Græsk drakme", Symbol: ""},
				currency.GTQ: cldr.Currency{DisplayName: "guatemalansk quetzal", Symbol: "Q"},
				currency.GWE: cldr.Currency{DisplayName: "Portugisisk guinea escudo", Symbol: ""},
				currency.GWP: cldr.Currency{DisplayName: "Guineansk peso", Symbol: ""},
				currency.GYD: cldr.Currency{DisplayName: "guyansk dollar", Symbol: "$"},
				currency.HKD: cldr.Currency{DisplayName: "hongkongsk dollar", Symbol: "HK$"},
				currency.HNL: cldr.Currency{DisplayName: "honduransk lempira", Symbol: "L"},
				currency.HRD: cldr.Currency{DisplayName: "Kroatisk dinar", Symbol: ""},
				currency.HRK: cldr.Currency{DisplayName: "kroatisk kuna", Symbol: "kn"},
				currency.HTG: cldr.Currency{DisplayName: "haitisk gourde", Symbol: ""},
				currency.HUF: cldr.Currency{DisplayName: "ungarsk forint", Symbol: "Ft"},
				currency.IDR: cldr.Currency{DisplayName: "indonesisk rupiah", Symbol: "Rp"},
				currency.IEP: cldr.Currency{DisplayName: "Irsk pund", Symbol: ""},
				currency.ILP: cldr.Currency{DisplayName: "Israelsk pund", Symbol: ""},
				currency.ILR: cldr.Currency{DisplayName: "israelsk shekel (1980–1985)", Symbol: ""},
				currency.ILS: cldr.Currency{DisplayName: "ny israelsk shekel", Symbol: "₪"},
				currency.INR: cldr.Currency{DisplayName: "indisk rupee", Symbol: "₹"},
				currency.IQD: cldr.Currency{DisplayName: "irakisk dinar", Symbol: ""},
				currency.IRR: cldr.Currency{DisplayName: "iransk rial", Symbol: ""},
				currency.ISJ: cldr.Currency{DisplayName: "islandsk krone (1918–1981)", Symbol: ""},
				currency.ISK: cldr.Currency{DisplayName: "islandsk krone", Symbol: "kr."},
				currency.ITL: cldr.Currency{DisplayName: "Italiensk lire", Symbol: ""},
				currency.JMD: cldr.Currency{DisplayName: "jamaicansk dollar", Symbol: "$"},
				currency.JOD: cldr.Currency{DisplayName: "jordansk dinar", Symbol: ""},
				currency.JPY: cldr.Currency{DisplayName: "japansk yen", Symbol: "JP¥"},
				currency.KES: cldr.Currency{DisplayName: "kenyansk shilling", Symbol: ""},
				currency.KGS: cldr.Currency{DisplayName: "kirgisisk som", Symbol: "⃀"},
				currency.KHR: cldr.Currency{DisplayName: "cambodjansk riel", Symbol: "៛"},
				currency.KMF: cldr.Currency{DisplayName: "comorisk franc", Symbol: "CF"},
				currency.KPW: cldr.Currency{DisplayName: "nordkoreansk won", Symbol: "₩"},
				currency.KRH: cldr.Currency{DisplayName: "sydkoreansk hwan (1953–1962)", Symbol: ""},
				currency.KRO: cldr.Currency{DisplayName: "sydkoreansk won (1945–1953)", Symbol: ""},
				currency.KRW: cldr.Currency{DisplayName: "sydkoreansk won", Symbol: "₩"},
				currency.KWD: cldr.Currency{DisplayName: "kuwaitisk dinar", Symbol: ""},
				currency.KYD: cldr.Currency{DisplayName: "caymansk dollar", Symbol: "$"},
				currency.KZT: cldr.Currency{DisplayName: "kasakhisk tenge", Symbol: "₸"},
				currency.LAK: cldr.Currency{DisplayName: "laotisk kip", Symbol: "₭"},
				currency.LBP: cldr.Currency{DisplayName: "libanesisk pund", Symbol: "L£"},
				currency.LKR: cldr.Currency{DisplayName: "srilankansk rupee", Symbol: "Rs"},
				currency.LRD: cldr.Currency{DisplayName: "liberisk dollar", Symbol: "$"},
				currency.LSL: cldr.Currency{DisplayName: "lesothisk loti", Symbol: ""},
				currency.LTL: cldr.Currency{DisplayName: "Litauisk litas", Symbol: "Lt"},
				currency.LTT: cldr.Currency{DisplayName: "Litauisk talonas", Symbol: ""},
				currency.LUC: cldr.Currency{DisplayName: "Luxembourgsk konvertibel franc", Symbol: ""},
				currency.LUF: cldr.Currency{DisplayName: "Luxembourgsk franc", Symbol: ""},
				currency.LUL: cldr.Currency{DisplayName: "Luxembourgsk finansiel franc", Symbol: ""},
				currency.LVL: cldr.Currency{DisplayName: "Lettisk lat", Symbol: "Ls"},
				currency.LVR: cldr.Currency{DisplayName: "Lettisk rubel", Symbol: ""},
				currency.LYD: cldr.Currency{DisplayName: "libysk dinar", Symbol: ""},
				currency.MAD: cldr.Currency{DisplayName: "marokkansk dirham", Symbol: ""},
				currency.MAF: cldr.Currency{DisplayName: "Marokkansk franc", Symbol: ""},
				currency.MCF: cldr.Currency{DisplayName: "monegaskisk franc", Symbol: ""},
				currency.MDC: cldr.Currency{DisplayName: "moldovisk cupon", Symbol: ""},
				currency.MDL: cldr.Currency{DisplayName: "moldovisk leu", Symbol: ""},
				currency.MGA: cldr.Currency{DisplayName: "madagaskisk ariary", Symbol: "Ar"},
				currency.MGF: cldr.Currency{DisplayName: "Madagaskisk franc", Symbol: ""},
				currency.MKD: cldr.Currency{DisplayName: "makedonsk denar", Symbol: ""},
				currency.MKN: cldr.Currency{DisplayName: "makedonsk denar (1992–1993)", Symbol: ""},
				currency.MLF: cldr.Currency{DisplayName: "Malisk franc", Symbol: ""},
				currency.MMK: cldr.Currency{DisplayName: "myanmarsk kyat", Symbol: "K"},
				currency.MNT: cldr.Currency{DisplayName: "mongolsk tugrik", Symbol: "₮"},
				currency.MOP: cldr.Currency{DisplayName: "macaosk pataca", Symbol: ""},
				currency.MRO: cldr.Currency{DisplayName: "mauritansk ouguiya (1973–2017)", Symbol: ""},
				currency.MRU: cldr.Currency{DisplayName: "mauritansk ouguiya", Symbol: ""},
				currency.MTL: cldr.Currency{DisplayName: "Maltesisk lira", Symbol: ""},
				currency.MTP: cldr.Currency{DisplayName: "Maltesisk pund", Symbol: ""},
				currency.MUR: cldr.Currency{DisplayName: "mauritisk rupee", Symbol: "Rs"},
				currency.MVP: cldr.Currency{DisplayName: "maldivisk rupi (1947–1981)", Symbol: ""},
				currency.MVR: cldr.Currency{DisplayName: "maldivisk rufiyaa", Symbol: ""},
				currency.MWK: cldr.Currency{DisplayName: "malawisk kwacha", Symbol: ""},
				currency.MXN: cldr.Currency{DisplayName: "mexicansk peso", Symbol: "MX$"},
				currency.MXP: cldr.Currency{DisplayName: "Mexicansk silver peso (1861–1992)", Symbol: ""},
				currency.MYR: cldr.Currency{DisplayName: "malaysisk ringgit", Symbol: "RM"},
				currency.MZE: cldr.Currency{DisplayName: "Mozambiquisk escudo", Symbol: ""},
				currency.MZM: cldr.Currency{DisplayName: "Mozambiquisk metical (1980–2006)", Symbol: ""},
				currency.MZN: cldr.Currency{DisplayName: "mozambiquisk metical", Symbol: ""},
				currency.NAD: cldr.Currency{DisplayName: "namibisk dollar", Symbol: "$"},
				currency.NGN: cldr.Currency{DisplayName: "nigeriansk naira", Symbol: "₦"},
				currency.NIC: cldr.Currency{DisplayName: "Nicaraguansk cordoba (1988–1991)", Symbol: ""},
				currency.NIO: cldr.Currency{DisplayName: "nicaraguansk cordoba", Symbol: "C$"},
				currency.NLG: cldr.Currency{DisplayName: "Hollandsk guilder", Symbol: ""},
				currency.NOK: cldr.Currency{DisplayName: "norsk krone", Symbol: "kr."},
				currency.NPR: cldr.Currency{DisplayName: "nepalesisk rupee", Symbol: "Rs"},
				currency.NZD: cldr.Currency{DisplayName: "newzealandsk dollar", Symbol: "NZ$"},
				currency.OMR: cldr.Currency{DisplayName: "omansk rial", Symbol: ""},
				currency.PAB: cldr.Currency{DisplayName: "panamansk balboa", Symbol: ""},
				currency.PEI: cldr.Currency{DisplayName: "peruviansk inti", Symbol: ""},
				currency.PEN: cldr.Currency{DisplayName: "peruansk sol", Symbol: ""},
				currency.PES: cldr.Currency{DisplayName: "peruviansk sol (1863–1965)", Symbol: ""},
				currency.PGK: cldr.Currency{DisplayName: "papuansk kina", Symbol: ""},
				currency.PHP: cldr.Currency{DisplayName: "filippinsk peso", Symbol: "PHP"},
				currency.PKR: cldr.Currency{DisplayName: "pakistansk rupee", Symbol: "Rs"},
				currency.PLN: cldr.Currency{DisplayName: "polsk zloty", Symbol: "zł"},
				currency.PLZ: cldr.Currency{DisplayName: "Polsk zloty (1950–1995)", Symbol: ""},
				currency.PTE: cldr.Currency{DisplayName: "Portugisisk escudo", Symbol: ""},
				currency.PYG: cldr.Currency{DisplayName: "paraguaysk guarani", Symbol: "₲"},
				currency.QAR: cldr.Currency{DisplayName: "qatarsk rial", Symbol: ""},
				currency.RHD: cldr.Currency{DisplayName: "rhodesisk dollar", Symbol: ""},
				currency.ROL: cldr.Currency{DisplayName: "Rumænsk leu (1952–2006)", Symbol: ""},
				currency.RON: cldr.Currency{DisplayName: "rumænsk leu", Symbol: "L"},
				currency.RSD: cldr.Currency{DisplayName: "serbisk dinar", Symbol: ""},
				currency.RUB: cldr.Currency{DisplayName: "russisk rubel", Symbol: "₽"},
				currency.RUR: cldr.Currency{DisplayName: "Russisk rubel (1991–1998)", Symbol: ""},
				currency.RWF: cldr.Currency{DisplayName: "rwandisk franc", Symbol: "RF"},
				currency.SAR: cldr.Currency{DisplayName: "saudiarabisk riyal", Symbol: ""},
				currency.SBD: cldr.Currency{DisplayName: "salomonsk dollar", Symbol: "$"},
				currency.SCR: cldr.Currency{DisplayName: "seychellisk rupee", Symbol: ""},
				currency.SDD: cldr.Currency{DisplayName: "Sudansk dinar (1992–2007)", Symbol: ""},
				currency.SDG: cldr.Currency{DisplayName: "sudansk pund", Symbol: ""},
				currency.SDP: cldr.Currency{DisplayName: "Sudansk pund (1957–1998)", Symbol: ""},
				currency.SEK: cldr.Currency{DisplayName: "svensk krone", Symbol: "kr."},
				currency.SGD: cldr.Currency{DisplayName: "singaporeansk dollar", Symbol: "$"},
				currency.SHP: cldr.Currency{DisplayName: "pund fra Saint Helena", Symbol: "£"},
				currency.SIT: cldr.Currency{DisplayName: "Slovensk tolar", Symbol: ""},
				currency.SKK: cldr.Currency{DisplayName: "Slovakisk koruna", Symbol: ""},
				currency.SLE: cldr.Currency{DisplayName: "sierraleonsk leone", Symbol: ""},
				currency.SLL: cldr.Currency{DisplayName: "sierraleonsk leone (1964—2022)", Symbol: ""},
				currency.SOS: cldr.Currency{DisplayName: "somalisk shilling", Symbol: ""},
				currency.SRD: cldr.Currency{DisplayName: "surinamsk dollar", Symbol: "$"},
				currency.SRG: cldr.Currency{DisplayName: "Surinamsk guilder", Symbol: ""},
				currency.SSP: cldr.Currency{DisplayName: "sydsudansk pund", Symbol: "£"},
				currency.STD: cldr.Currency{DisplayName: "dobra fra Sao Tome og Principe (1977–2017)", Symbol: ""},
				currency.STN: cldr.Currency{DisplayName: "dobra fra Sao Tome og Principe", Symbol: "Db"},
				currency.SUR: cldr.Currency{DisplayName: "Sovjetisk rubel", Symbol: ""},
				currency.SVC: cldr.Currency{DisplayName: "Salvadoransk colon", Symbol: ""},
				currency.SYP: cldr.Currency{DisplayName: "syrisk pund", Symbol: "£"},
				currency.SZL: cldr.Currency{DisplayName: "swazilandsk lilangeni", Symbol: ""},
				currency.THB: cldr.Currency{DisplayName: "thailandsk baht", Symbol: "฿"},
				currency.TJR: cldr.Currency{DisplayName: "Tadsjikisk rubel", Symbol: ""},
				currency.TJS: cldr.Currency{DisplayName: "tadsjikisk somoni", Symbol: ""},
				currency.TMM: cldr.Currency{DisplayName: "Turkmensk manat (1993–2009)", Symbol: ""},
				currency.TMT: cldr.Currency{DisplayName: "turkmensk manat", Symbol: ""},
				currency.TND: cldr.Currency{DisplayName: "tunesisk dinar", Symbol: ""},
				currency.TOP: cldr.Currency{DisplayName: "tongansk paʻanga", Symbol: "T$"},
				currency.TPE: cldr.Currency{DisplayName: "Escudo fra Timor", Symbol: ""},
				currency.TRL: cldr.Currency{DisplayName: "Tyrkisk lire (1922–2005)", Symbol: ""},
				currency.TRY: cldr.Currency{DisplayName: "tyrkisk lira", Symbol: "₺"},
				currency.TTD: cldr.Currency{DisplayName: "trinidadisk dollar", Symbol: "$"},
				currency.TWD: cldr.Currency{DisplayName: "ny taiwansk dollar", Symbol: "NT$"},
				currency.TZS: cldr.Currency{DisplayName: "tanzanisk shilling", Symbol: ""},
				currency.UAH: cldr.Currency{DisplayName: "ukrainsk grynia", Symbol: "₴"},
				currency.UAK: cldr.Currency{DisplayName: "Ukrainsk karbovanetz", Symbol: ""},
				currency.UGS: cldr.Currency{DisplayName: "Ugandisk shilling (1966–1987)", Symbol: ""},
				currency.UGX: cldr.Currency{DisplayName: "ugandisk shilling", Symbol: ""},
				currency.USD: cldr.Currency{DisplayName: "amerikansk dollar", Symbol: "US$"},
				currency.USN: cldr.Currency{DisplayName: "Amerikansk dollar (næste dag)", Symbol: ""},
				currency.USS: cldr.Currency{DisplayName: "Amerikansk dollar (samme dag)", Symbol: ""},
				currency.UYP: cldr.Currency{DisplayName: "Uruguayansk peso (1975–1993)", Symbol: ""},
				currency.UYU: cldr.Currency{DisplayName: "uruguayansk peso", Symbol: "$"},
				currency.UZS: cldr.Currency{DisplayName: "usbekisk sum", Symbol: ""},
				currency.VEB: cldr.Currency{DisplayName: "Venezuelansk bolivar (1871–2008)", Symbol: ""},
				currency.VEF: cldr.Currency{DisplayName: "venezuelansk bolivar (2008–2018)", Symbol: "Bs"},
				currency.VES: cldr.Currency{DisplayName: "venezuelansk bolivar", Symbol: ""},
				currency.VND: cldr.Currency{DisplayName: "vietnamesisk dong", Symbol: "₫"},
				currency.VNN: cldr.Currency{DisplayName: "vietnamesisk dong (1978–1985)", Symbol: ""},
				currency.VUV: cldr.Currency{DisplayName: "vanuaisk vatu", Symbol: ""},
				currency.WST: cldr.Currency{DisplayName: "samoansk tala", Symbol: ""},
				currency.XAF: cldr.Currency{DisplayName: "CFA-franc (BEAC)", Symbol: "FCFA"},
				currency.XAG: cldr.Currency{DisplayName: "Sølv", Symbol: ""},
				currency.XAU: cldr.Currency{DisplayName: "Guld", Symbol: ""},
				currency.XBA: cldr.Currency{DisplayName: "EURCO", Symbol: ""},
				currency.XBB: cldr.Currency{DisplayName: "EMU", Symbol: ""},
				currency.XCD: cldr.Currency{DisplayName: "østkaribisk dollar", Symbol: "EC$"},
				currency.XCG: cldr.Currency{DisplayName: "", Symbol: "Cg."},
				currency.XDR: cldr.Currency{DisplayName: "SDR", Symbol: ""},
				currency.XEU: cldr.Currency{DisplayName: "ECU", Symbol: ""},
				currency.XFO: cldr.Currency{DisplayName: "Fransk guldfranc", Symbol: ""},
				currency.XFU: cldr.Currency{DisplayName: "Fransk UIC-franc", Symbol: ""},
				currency.XOF: cldr.Currency{DisplayName: "CFA-franc BCEAO", Symbol: "F\u202fCFA"},
				currency.XPD: cldr.Currency{DisplayName: "Palladium", Symbol: ""},
				currency.XPF: cldr.Currency{DisplayName: "CFP-franc", Symbol: "CFPF"},
				currency.XPT: cldr.Currency{DisplayName: "Platin", Symbol: ""},
				currency.XRE: cldr.Currency{DisplayName: "RINET-fond", Symbol: ""},
				currency.XTS: cldr.Currency{DisplayName: "testvalutakode", Symbol: ""},
				currency.XXX: cldr.Currency{DisplayName: "ukendt valuta", Symbol: "¤"},
				currency.YDD: cldr.Currency{DisplayName: "Yemenitisk dinar", Symbol: ""},
				currency.YER: cldr.Currency{DisplayName: "yemenitisk rial", Symbol: ""},
				currency.YUD: cldr.Currency{DisplayName: "Jugoslavisk hard dinar (1966–1990)", Symbol: ""},
				currency.YUM: cldr.Currency{DisplayName: "Jugoslavisk noviy dinar (1994–2002)", Symbol: ""},
				currency.YUN: cldr.Currency{DisplayName: "Jugoslavisk konvertibel dinar (1990–1992)", Symbol: ""},
				currency.YUR: cldr.Currency{DisplayName: "jugoslavisk reformeret dinar (1992–1993)", Symbol: ""},
				currency.ZAL: cldr.Currency{DisplayName: "Sydafrikansk rand (financial)", Symbol: ""},
				currency.ZAR: cldr.Currency{DisplayName: "sydafrikansk rand", Symbol: "R"},
				currency.ZMK: cldr.Currency{DisplayName: "Zambisk kwacha (1968–2012)", Symbol: ""},
				currency.ZMW: cldr.Currency{DisplayName: "zambisk kwacha", Symbol: "ZK"},
				currency.ZRN: cldr.Currency{DisplayName: "Ny zairisk zaire (1993–1998)", Symbol: ""},
				currency.ZRZ: cldr.Currency{DisplayName: "Zairisk zaire (1971–1993)", Symbol: ""},
				currency.ZWD: cldr.Currency{DisplayName: "Zimbabwisk dollar (1980–2008)", Symbol: ""},
				currency.ZWL: cldr.Currency{DisplayName: "Zimbabwisk dollar (2009)", Symbol: ""},
				currency.ZWR: cldr.Currency{DisplayName: "Zimbabwisk dollar (2008)", Symbol: ""},
			},
		},
		Display: cldr.LocaleDisplayPattern{Pattern: "{0} ({1})", Separator: "{0}, {1}", KeyTypePattern: "{0}: {1}"},
		Languages: cldr.Languages{
			language.AA:      "afar",
			language.AB:      "abkhasisk",
			language.ACE:     "achinesisk",
			language.ACH:     "acoli",
			language.ADA:     "adangme",
			language.ADY:     "adyghe",
			language.AE:      "avestan",
			language.AF:      "afrikaans",
			language.AFH:     "afrihili",
			language.AGQ:     "aghem",
			language.AIN:     "ainu",
			language.AK:      "akan",
			language.AKK:     "akkadisk",
			language.ALE:     "aleutisk",
			language.ALT:     "sydaltaisk",
			language.AM:      "amharisk",
			language.AN:      "aragonsk",
			language.ANG:     "oldengelsk",
			language.ANN:     "obolo",
			language.ANP:     "angika",
			language.AR:      "arabisk",
			language.AR_001:  "moderne standardarabisk",
			language.ARC:     "aramæisk",
			language.ARN:     "mapudungun",
			language.ARP:     "arapaho",
			language.ARS:     "arabisk, najdi",
			language.ARW:     "arawak",
			language.AS:      "assamesisk",
			language.ASA:     "asu",
			language.AST:     "asturisk",
			language.ATJ:     "atikamekw",
			language.AV:      "avarisk",
			language.AWA:     "awadhi",
			language.AY:      "aymara",
			language.AZ:      "azeri",
			language.BA:      "bashkir",
			language.BAL:     "baluchi",
			language.BAN:     "balinesisk",
			language.BAS:     "basaa",
			language.BAX:     "bamun",
			language.BBJ:     "ghomala",
			language.BE:      "belarusisk",
			language.BEJ:     "beja",
			language.BEM:     "bemba",
			language.BEZ:     "bena",
			language.BFD:     "bafut",
			language.BG:      "bulgarsk",
			language.BGC:     "harianvi",
			language.BGN:     "vestbaluchi",
			language.BHO:     "bhojpuri",
			language.BI:      "bislama",
			language.BIK:     "bikol",
			language.BIN:     "bini",
			language.BKM:     "kom",
			language.BLA:     "siksika",
			language.BLO:     "anii",
			language.BM:      "bambara",
			language.BN:      "bengali",
			language.BO:      "tibetansk",
			language.BR:      "bretonsk",
			language.BRA:     "braj",
			language.BRX:     "bodo",
			language.BS:      "bosnisk",
			language.BSS:     "bakossi",
			language.BUA:     "buriatisk",
			language.BUG:     "buginesisk",
			language.BUM:     "bulu",
			language.BYN:     "blin",
			language.BYV:     "medumba",
			language.CA:      "catalansk",
			language.CAD:     "caddo",
			language.CAR:     "caribisk",
			language.CAY:     "cayuga",
			language.CCH:     "atsam",
			language.CCP:     "chakma",
			language.CE:      "tjetjensk",
			language.CEB:     "cebuano",
			language.CGG:     "chiga",
			language.CH:      "chamorro",
			language.CHB:     "chibcha",
			language.CHG:     "chagatai",
			language.CHK:     "chuukese",
			language.CHM:     "mari",
			language.CHN:     "chinook",
			language.CHO:     "choctaw",
			language.CHP:     "chipewyan",
			language.CHR:     "cherokee",
			language.CHY:     "cheyenne",
			language.CKB:     "centralkurdisk",
			language.CLC:     "chilcotin",
			language.CO:      "korsikansk",
			language.COP:     "koptisk",
			language.CR:      "cree",
			language.CRG:     "michif",
			language.CRH:     "krimtatarisk",
			language.CRJ:     "sydøstcree",
			language.CRK:     "plains cree",
			language.CRL:     "nordøstcree",
			language.CRM:     "Moose-cree",
			language.CRR:     "carolina algonquisk",
			language.CRS:     "seselwa (kreol-fransk)",
			language.CS:      "tjekkisk",
			language.CSB:     "kasjubisk",
			language.CSW:     "swampy cree",
			language.CU:      "kirkeslavisk",
			language.CV:      "tjuvasjisk",
			language.CY:      "walisisk",
			language.DA:      "dansk",
			language.DAK:     "dakota",
			language.DAR:     "dargwa",
			language.DAV:     "taita",
			language.DE:      "tysk",
			language.DE_AT:   "østrigsk tysk",
			language.DE_CH:   "schweizerhøjtysk",
			language.DEL:     "delaware",
			language.DEN:     "athapaskisk",
			language.DGR:     "dogrib",
			language.DIN:     "dinka",
			language.DJE:     "zarma",
			language.DOI:     "dogri",
			language.DSB:     "nedersorbisk",
			language.DUA:     "duala",
			language.DUM:     "middelhollandsk",
			language.DV:      "divehi",
			language.DYO:     "jola-fonyi",
			language.DYU:     "dyula",
			language.DZ:      "dzongkha",
			language.DZG:     "dazaga",
			language.EBU:     "kiembu",
			language.EE:      "ewe",
			language.EFI:     "efik",
			language.EGY:     "oldegyptisk",
			language.EKA:     "ekajuk",
			language.EL:      "græsk",
			language.ELX:     "elamitisk",
			language.EN:      "engelsk",
			language.EN_AU:   "australsk engelsk",
			language.EN_CA:   "canadisk engelsk",
			language.EN_GB:   "britisk engelsk",
			language.EN_US:   "amerikansk engelsk",
			language.ENM:     "middelengelsk",
			language.EO:      "esperanto",
			language.ES:      "spansk",
			language.ES_419:  "latinamerikansk spansk",
			language.ES_ES:   "europæisk spansk",
			language.ES_MX:   "mexicansk spansk",
			language.ET:      "estisk",
			language.EU:      "baskisk",
			language.EWO:     "ewondo",
			language.FA:      "persisk",
			language.FA_AF:   "dari",
			language.FAN:     "fang",
			language.FAT:     "fanti",
			language.FF:      "fulah",
			language.FI:      "finsk",
			language.FIL:     "filippinsk",
			language.FJ:      "fijiansk",
			language.FO:      "færøsk",
			language.FON:     "fon",
			language.FR:      "fransk",
			language.FR_CA:   "canadisk fransk",
			language.FR_CH:   "schweizisk fransk",
			language.FRC:     "cajunfransk",
			language.FRM:     "middelfransk",
			language.FRO:     "oldfransk",
			language.FRR:     "nordfrisisk",
			language.FRS:     "østfrisisk",
			language.FUR:     "friulisk",
			language.FY:      "vestfrisisk",
			language.GA:      "irsk",
			language.GAA:     "ga",
			language.GAG:     "gagauzisk",
			language.GAN:     "gan-kinesisk",
			language.GAY:     "gayo",
			language.GBA:     "gbaya",
			language.GD:      "skotsk gælisk",
			language.GEZ:     "geez",
			language.GIL:     "gilbertesisk",
			language.GL:      "galicisk",
			language.GMH:     "middelhøjtysk",
			language.GN:      "guarani",
			language.GOH:     "oldhøjtysk",
			language.GON:     "gondi",
			language.GOR:     "gorontalo",
			language.GOT:     "gotisk",
			language.GRB:     "grebo",
			language.GRC:     "oldgræsk",
			language.GSW:     "schweizertysk",
			language.GU:      "gujarati",
			language.GUZ:     "gusii",
			language.GV:      "manx",
			language.GWI:     "gwichin",
			language.HA:      "hausa",
			language.HAI:     "haida",
			language.HAK:     "hakka-kinesisk",
			language.HAW:     "hawaiiansk",
			language.HAX:     "sydhaida",
			language.HE:      "hebraisk",
			language.HI:      "hindi",
			language.HI_LATN: "hinglish",
			language.HIL:     "hiligaynon",
			language.HIT:     "hittitisk",
			language.HMN:     "hmong",
			language.HO:      "hirimotu",
			language.HR:      "kroatisk",
			language.HSB:     "øvresorbisk",
			language.HSN:     "xiang-kinesisk",
			language.HT:      "haitisk",
			language.HU:      "ungarsk",
			language.HUP:     "hupa",
			language.HUR:     "halkomelem",
			language.HY:      "armensk",
			language.HZ:      "herero",
			language.IA:      "interlingua",
			language.IBA:     "iban",
			language.IBB:     "ibibio",
			language.ID:      "indonesisk",
			language.IE:      "interlingue",
			language.IG:      "igbo",
			language.II:      "sichuan yi",
			language.IK:      "inupiaq",
			language.IKT:     "vestcanadisk inuktitut",
			language.ILO:     "iloko",
			language.INH:     "ingush",
			language.IO:      "ido",
			language.IS:      "islandsk",
			language.IT:      "italiensk",
			language.IU:      "inuktitut",
			language.JA:      "japansk",
			language.JBO:     "lojban",
			language.JGO:     "ngomba",
			language.JMC:     "machame",
			language.JPR:     "jødisk-persisk",
			language.JRB:     "jødisk-arabisk",
			language.JV:      "javanesisk",
			language.KA:      "georgisk",
			language.KAA:     "karakalpakisk",
			language.KAB:     "kabylsk",
			language.KAC:     "kachin",
			language.KAJ:     "jju",
			language.KAM:     "kamba",
			language.KAW:     "kawi",
			language.KBD:     "kabardian",
			language.KBL:     "kanembu",
			language.KCG:     "tyap",
			language.KDE:     "makonde",
			language.KEA:     "kapverdisk",
			language.KFO:     "koro",
			language.KG:      "kongo",
			language.KGP:     "kaingang",
			language.KHA:     "khasi",
			language.KHO:     "khotanesisk",
			language.KHQ:     "koyra-chiini",
			language.KI:      "kikuyu",
			language.KJ:      "kuanyama",
			language.KK:      "kasakhisk",
			language.KKJ:     "kako",
			language.KL:      "grønlandsk",
			language.KLN:     "kalenjin",
			language.KM:      "khmer",
			language.KMB:     "kimbundu",
			language.KN:      "kannada",
			language.KO:      "koreansk",
			language.KOI:     "komi-permjakisk",
			language.KOK:     "konkani",
			language.KOS:     "kosraean",
			language.KPE:     "kpelle",
			language.KR:      "kanuri",
			language.KRC:     "karatjai-balkar",
			language.KRL:     "karelsk",
			language.KRU:     "kurukh",
			language.KS:      "kashmiri",
			language.KSB:     "shambala",
			language.KSF:     "bafia",
			language.KSH:     "kölsch",
			language.KU:      "kurdisk",
			language.KUM:     "kymyk",
			language.KUT:     "kutenaj",
			language.KV:      "komi",
			language.KW:      "cornisk",
			language.KWK:     "kwakʼwala",
			language.KXV:     "kuvi",
			language.KY:      "kirgisisk",
			language.LA:      "latin",
			language.LAD:     "ladino",
			language.LAG:     "langi",
			language.LAH:     "lahnda",
			language.LAM:     "lamba",
			language.LB:      "luxembourgsk",
			language.LEZ:     "lezghian",
			language.LG:      "ganda",
			language.LI:      "limburgsk",
			language.LIJ:     "ligurisk",
			language.LIL:     "lillooet",
			language.LKT:     "lakota",
			language.LMO:     "lombardisk",
			language.LN:      "lingala",
			language.LO:      "lao",
			language.LOL:     "mongo",
			language.LOU:     "Louisiana-kreolsk",
			language.LOZ:     "lozi",
			language.LRC:     "nordluri",
			language.LSM:     "saamia",
			language.LT:      "litauisk",
			language.LU:      "luba-Katanga",
			language.LUA:     "luba-Lulua",
			language.LUI:     "luiseno",
			language.LUN:     "lunda",
			language.LUS:     "lushai",
			language.LUY:     "luyana",
			language.LV:      "lettisk",
			language.MAD:     "madurese",
			language.MAF:     "mafa",
			language.MAG:     "magahi",
			language.MAI:     "maithili",
			language.MAK:     "makasar",
			language.MAN:     "mandingo",
			language.MAS:     "masai",
			language.MDE:     "maba",
			language.MDF:     "moksha",
			language.MDR:     "mandar",
			language.MEN:     "mende",
			language.MER:     "meru",
			language.MFE:     "morisyen",
			language.MG:      "malagassisk",
			language.MGA:     "middelirsk",
			language.MGH:     "makhuwa-meetto",
			language.MGO:     "meta",
			language.MH:      "marshallese",
			language.MI:      "maori",
			language.MIC:     "micmac",
			language.MIN:     "minangkabau",
			language.MK:      "makedonsk",
			language.ML:      "malayalam",
			language.MN:      "mongolsk",
			language.MNC:     "manchu",
			language.MNI:     "manipuri",
			language.MOE:     "innu-aimun",
			language.MOH:     "mohawk",
			language.MOS:     "mossi",
			language.MR:      "marathi",
			language.MS:      "malajisk",
			language.MT:      "maltesisk",
			language.MUA:     "mundang",
			language.MUL:     "flere sprog",
			language.MUS:     "creek",
			language.MWL:     "mirandesisk",
			language.MWR:     "marwari",
			language.MY:      "burmesisk",
			language.MYE:     "myene",
			language.MYV:     "erzya",
			language.MZN:     "mazenisk",
			language.NA:      "nauru",
			language.NAN:     "min-kinesisk",
			language.NAP:     "napolitansk",
			language.NAQ:     "nama",
			language.NB:      "bokmål",
			language.ND:      "nordndebele",
			language.NDS:     "nedertysk",
			language.NDS_NL:  "plattysk (Holland)",
			language.NE:      "nepalesisk",
			language.NEW:     "newari",
			language.NG:      "ndonga",
			language.NIA:     "nias",
			language.NIU:     "niueansk",
			language.NL:      "nederlandsk",
			language.NL_BE:   "flamsk",
			language.NMG:     "kwasio",
			language.NN:      "nynorsk",
			language.NNH:     "ngiemboon",
			language.NO:      "norsk",
			language.NOG:     "nogai",
			language.NON:     "oldislandsk",
			language.NQO:     "n’ko",
			language.NR:      "sydndebele",
			language.NSO:     "nordsotho",
			language.NUS:     "nuer",
			language.NV:      "navajo",
			language.NWC:     "klassisk newarisk",
			language.NY:      "nyanja",
			language.NYM:     "nyamwezi",
			language.NYN:     "nyankole",
			language.NYO:     "nyoro-sprog",
			language.NZI:     "nzima",
			language.OC:      "occitansk",
			language.OJ:      "ojibwa",
			language.OJB:     "nordvestojibwa",
			language.OJC:     "centralojibwa",
			language.OJS:     "oji-cree",
			language.OJW:     "vestojibwa",
			language.OKA:     "okanagan",
			language.OM:      "oromo",
			language.OR:      "oriya",
			language.OS:      "ossetisk",
			language.OSA:     "osage",
			language.OTA:     "osmannisk tyrkisk",
			language.PA:      "punjabi",
			language.PAG:     "pangasinan",
			language.PAL:     "pahlavi",
			language.PAM:     "pampanga",
			language.PAP:     "papiamento",
			language.PAU:     "palauansk",
			language.PCM:     "nigeriansk pidgin",
			language.PEO:     "oldpersisk",
			language.PHN:     "fønikisk",
			language.PI:      "pali",
			language.PIS:     "pijin",
			language.PL:      "polsk",
			language.PON:     "ponape",
			language.PQM:     "maliseet-passamaquoddy",
			language.PRG:     "preussisk",
			language.PRO:     "oldprovencalsk",
			language.PS:      "pushto",
			language.PT:      "portugisisk",
			language.PT_BR:   "brasiliansk portugisisk",
			language.PT_PT:   "europæisk portugisisk",
			language.QU:      "quechua",
			language.QUC:     "quiché",
			language.RAJ:     "rajasthani",
			language.RAP:     "rapanui",
			language.RAR:     "rarotonga",
			language.RHG:     "rohingya",
			language.RM:      "rætoromansk",
			language.RN:      "rundi",
			language.RO:      "rumænsk",
			language.RO_MD:   "moldovisk",
			language.ROF:     "rombo",
			language.ROM:     "romani",
			language.RU:      "russisk",
			language.RUP:     "arumænsk",
			language.RW:      "kinyarwanda",
			language.RWK:     "rwa",
			language.SA:      "sanskrit",
			language.SAD:     "sandawe",
			language.SAH:     "jakutisk",
			language.SAM:     "samaritansk aramæisk",
			language.SAQ:     "samburu",
			language.SAS:     "sasak",
			language.SAT:     "santali",
			language.SBA:     "ngambay",
			language.SBP:     "sangu",
			language.SC:      "sardinsk",
			language.SCN:     "siciliansk",
			language.SCO:     "skotsk",
			language.SD:      "sindhi",
			language.SDH:     "sydkurdisk",
			language.SE:      "nordsamisk",
			language.SEE:     "seneca",
			language.SEH:     "sena",
			language.SEL:     "selkupisk",
			language.SES:     "koyraboro senni",
			language.SG:      "sango",
			language.SGA:     "oldirsk",
			language.SH:      "serbokroatisk",
			language.SHI:     "tachelhit",
			language.SHN:     "shan",
			language.SHU:     "tchadisk arabisk",
			language.SI:      "singalesisk",
			language.SID:     "sidamo",
			language.SK:      "slovakisk",
			language.SL:      "slovensk",
			language.SLH:     "sydlushootseed",
			language.SM:      "samoansk",
			language.SMA:     "sydsamisk",
			language.SMJ:     "lulesamisk",
			language.SMN:     "enaresamisk",
			language.SMS:     "skoltesamisk",
			language.SN:      "shona",
			language.SNK:     "soninke",
			language.SO:      "somali",
			language.SOG:     "sogdiansk",
			language.SQ:      "albansk",
			language.SR:      "serbisk",
			language.SRN:     "sranan tongo",
			language.SRR:     "serer",
			language.SS:      "swati",
			language.SSY:     "saho",
			language.ST:      "sydsotho",
			language.STR:     "straits salish",
			language.SU:      "sundanesisk",
			language.SUK:     "sukuma",
			language.SUS:     "susu",
			language.SUX:     "sumerisk",
			language.SV:      "svensk",
			language.SW:      "swahili",
			language.SW_CD:   "congolesisk swahili",
			language.SWB:     "comorisk",
			language.SYC:     "klassisk syrisk",
			language.SYR:     "syrisk",
			language.SZL:     "schlesisk",
			language.TA:      "tamil",
			language.TCE:     "sydtutchone",
			language.TE:      "telugu",
			language.TEM:     "temne",
			language.TEO:     "teso",
			language.TER:     "tereno",
			language.TET:     "tetum",
			language.TG:      "tadsjikisk",
			language.TGX:     "tagish",
			language.TH:      "thai",
			language.THT:     "tahltan",
			language.TI:      "tigrinya",
			language.TIG:     "tigre",
			language.TIV:     "tivi",
			language.TK:      "turkmensk",
			language.TKL:     "tokelau",
			language.TL:      "tagalog",
			language.TLH:     "klingon",
			language.TLI:     "tlingit",
			language.TMH:     "tamashek",
			language.TN:      "tswana",
			language.TO:      "tongansk",
			language.TOG:     "nyasa tongansk",
			language.TOK:     "toki pona",
			language.TPI:     "tok pisin",
			language.TR:      "tyrkisk",
			language.TRV:     "taroko",
			language.TS:      "tsonga",
			language.TSI:     "tsimshisk",
			language.TT:      "tatarisk",
			language.TTM:     "nordtutchone",
			language.TUM:     "tumbuka",
			language.TVL:     "tuvaluansk",
			language.TW:      "twi",
			language.TWQ:     "tasawaq",
			language.TY:      "tahitiansk",
			language.TYV:     "tuvinian",
			language.TZM:     "centralmarokkansk tamazight",
			language.UDM:     "udmurt",
			language.UG:      "uighurisk",
			language.UGA:     "ugaristisk",
			language.UK:      "ukrainsk",
			language.UMB:     "umbundu",
			language.UND:     "ukendt sprog",
			language.UR:      "urdu",
			language.UZ:      "usbekisk",
			language.VE:      "venda",
			language.VEC:     "venetiansk",
			language.VI:      "vietnamesisk",
			language.VMW:     "makhuwa",
			language.VO:      "volapyk",
			language.VOT:     "votisk",
			language.VUN:     "vunjo",
			language.WA:      "vallonsk",
			language.WAE:     "walsertysk",
			language.WAL:     "walamo",
			language.WAR:     "waray",
			language.WAS:     "washo",
			language.WBP:     "walbiri",
			language.WO:      "wolof",
			language.WUU:     "wu-kinesisk",
			language.XAL:     "kalmyk",
			language.XH:      "xhosa",
			language.XNR:     "kangri",
			language.XOG:     "soga",
			language.YAO:     "yao",
			language.YAP:     "yapese",
			language.YAV:     "yangben",
			language.YBB:     "yemba",
			language.YI:      "jiddisch",
			language.YO:      "yoruba",
			language.YRL:     "nheengatu",
			language.YUE:     "kantonesisk (Kina)",
			language.ZA:      "zhuang",
			language.ZAP:     "zapotec",
			language.ZBL:     "blissymboler",
			language.ZEN:     "zenaga",
			language.ZGH:     "tamazight",
			language.ZH:      "mandarin (Kina)",
			language.ZH_HANS: "forenklet kinesisk",
			language.ZH_HANT: "traditionelt kinesisk",
			language.ZU:      "zulu",
			language.ZUN:     "zuni",
			language.ZXX:     "intet sprogligt indhold",
			language.ZZA:     "zaza",
		},
		Territories: cldr.Territories{
			territory.V_001: "Verden",
			territory.V_002: "Afrika",
			territory.V_003: "Nordamerika",
			territory.V_005: "Sydamerika",
			territory.V_009: "Oceanien",
			territory.V_011: "Vestafrika",
			territory.V_013: "Mellemamerika",
			territory.V_014: "Østafrika",
			territory.V_015: "Nordafrika",
			territory.V_017: "Centralafrika",
			territory.V_018: "Det sydlige Afrika",
			territory.V_019: "Nord-, Mellem- og Sydamerika",
			territory.V_021: "Det nordlige Amerika",
			territory.V_029: "Caribien",
			territory.V_030: "Østasien",
			territory.V_034: "Sydasien",
			territory.V_035: "Sydøstasien",
			territory.V_039: "Sydeuropa",
			territory.V_053: "Australasien",
			territory.V_054: "Melanesien",
			territory.V_057: "Mikronesiske område",
			territory.V_061: "Polynesien",
			territory.V_142: "Asien",
			territory.V_143: "Centralasien",
			territory.V_145: "Vestasien",
			territory.V_150: "Europa",
			territory.V_151: "Østeuropa",
			territory.V_154: "Nordeuropa",
			territory.V_155: "Vesteuropa",
			territory.V_202: "Subsaharisk Afrika",
			territory.V_419: "Latinamerika",
			territory.AC:    "Ascensionøen",
			territory.AD:    "Andorra",
			territory.AE:    "De Forenede Arabiske Emirater",
			territory.AF:    "Afghanistan",
			territory.AG:    "Antigua og Barbuda",
			territory.AI:    "Anguilla",
			territory.AL:    "Albanien",
			territory.AM:    "Armenien",
			territory.AO:    "Angola",
			territory.AQ:    "Antarktis",
			territory.AR:    "Argentina",
			territory.AS:    "Amerikansk Samoa",
			territory.AT:    "Østrig",
			territory.AU:    "Australien",
			territory.AW:    "Aruba",
			territory.AX:    "Åland",
			territory.AZ:    "Aserbajdsjan",
			territory.BA:    "Bosnien-Hercegovina",
			territory.BB:    "Barbados",
			territory.BD:    "Bangladesh",
			territory.BE:    "Belgien",
			territory.BF:    "Burkina Faso",
			territory.BG:    "Bulgarien",
			territory.BH:    "Bahrain",
			territory.BI:    "Burundi",
			territory.BJ:    "Benin",
			territory.BL:    "Saint Barthélemy",
			territory.BM:    "Bermuda",
			territory.BN:    "Brunei",
			territory.BO:    "Bolivia",
			territory.BQ:    "De tidligere Nederlandske Antiller",
			territory.BR:    "Brasilien",
			territory.BS:    "Bahamas",
			territory.BT:    "Bhutan",
			territory.BV:    "Bouvetøen",
			territory.BW:    "Botswana",
			territory.BY:    "Belarus",
			territory.BZ:    "Belize",
			territory.CA:    "Canada",
			territory.CC:    "Cocosøerne",
			territory.CD:    "Congo-Kinshasa",
			territory.CF:    "Den Centralafrikanske Republik",
			territory.CG:    "Congo-Brazzaville",
			territory.CH:    "Schweiz",
			territory.CI:    "Elfenbenskysten",
			territory.CK:    "Cookøerne",
			territory.CL:    "Chile",
			territory.CM:    "Cameroun",
			territory.CN:    "Kina",
			territory.CO:    "Colombia",
			territory.CP:    "Clippertonøen",
			territory.CR:    "Costa Rica",
			territory.CU:    "Cuba",
			territory.CV:    "Kap Verde",
			territory.CW:    "Curaçao",
			territory.CX:    "Juleøen",
			territory.CY:    "Cypern",
			territory.CZ:    "Tjekkiet",
			territory.DE:    "Tyskland",
			territory.DG:    "Diego Garcia",
			territory.DJ:    "Djibouti",
			territory.DK:    "Danmark",
			territory.DM:    "Dominica",
			territory.DO:    "Den Dominikanske Republik",
			territory.DZ:    "Algeriet",
			territory.EA:    "Ceuta og Melilla",
			territory.EC:    "Ecuador",
			territory.EE:    "Estland",
			territory.EG:    "Egypten",
			territory.EH:    "Vestsahara",
			territory.ER:    "Eritrea",
			territory.ES:    "Spanien",
			territory.ET:    "Etiopien",
			territory.EU:    "Den Europæiske Union",
			territory.EZ:    "eurozonen",
			territory.FI:    "Finland",
			territory.FJ:    "Fiji",
			territory.FK:    "Falklandsøerne",
			territory.FM:    "Mikronesien",
			territory.FO:    "Færøerne",
			territory.FR:    "Frankrig",
			territory.GA:    "Gabon",
			territory.GB:    "Storbritannien",
			territory.GD:    "Grenada",
			territory.GE:    "Georgien",
			territory.GF:    "Fransk Guyana",
			territory.GG:    "Guernsey",
			territory.GH:    "Ghana",
			territory.GI:    "Gibraltar",
			territory.GL:    "Grønland",
			territory.GM:    "Gambia",
			territory.GN:    "Guinea",
			territory.GP:    "Guadeloupe",
			territory.GQ:    "Ækvatorialguinea",
			territory.GR:    "Grækenland",
			territory.GS:    "South Georgia og De Sydlige Sandwichøer",
			territory.GT:    "Guatemala",
			territory.GU:    "Guam",
			territory.GW:    "Guinea-Bissau",
			territory.GY:    "Guyana",
			territory.HK:    "SAR Hongkong",
			territory.HM:    "Heard Island og McDonald Islands",
			territory.HN:    "Honduras",
			territory.HR:    "Kroatien",
			territory.HT:    "Haiti",
			territory.HU:    "Ungarn",
			territory.IC:    "Kanariske øer",
			territory.ID:    "Indonesien",
			territory.IE:    "Irland",
			territory.IL:    "Israel",
			territory.IM:    "Isle of Man",
			territory.IN:    "Indien",
			territory.IO:    "Det Britiske Territorium i Det Indiske Ocean",
			territory.IQ:    "Irak",
			territory.IR:    "Iran",
			territory.IS:    "Island",
			territory.IT:    "Italien",
			territory.JE:    "Jersey",
			territory.JM:    "Jamaica",
			territory.JO:    "Jordan",
			territory.JP:    "Japan",
			territory.KE:    "Kenya",
			territory.KG:    "Kirgisistan",
			territory.KH:    "Cambodja",
			territory.KI:    "Kiribati",
			territory.KM:    "Comorerne",
			territory.KN:    "Saint Kitts og Nevis",
			territory.KP:    "Nordkorea",
			territory.KR:    "Sydkorea",
			territory.KW:    "Kuwait",
			territory.KY:    "Caymanøerne",
			territory.KZ:    "Kasakhstan",
			territory.LA:    "Laos",
			territory.LB:    "Libanon",
			territory.LC:    "Saint Lucia",
			territory.LI:    "Liechtenstein",
			territory.LK:    "Sri Lanka",
			territory.LR:    "Liberia",
			territory.LS:    "Lesotho",
			territory.LT:    "Litauen",
			territory.LU:    "Luxembourg",
			territory.LV:    "Letland",
			territory.LY:    "Libyen",
			territory.MA:    "Marokko",
			territory.MC:    "Monaco",
			territory.MD:    "Moldova",
			territory.ME:    "Montenegro",
			territory.MF:    "Saint Martin",
			territory.MG:    "Madagaskar",
			territory.MH:    "Marshalløerne",
			territory.MK:    "Nordmakedonien",
			territory.ML:    "Mali",
			territory.MM:    "Myanmar (Burma)",
			territory.MN:    "Mongoliet",
			territory.MO:    "SAR Macao",
			territory.MP:    "Nordmarianerne",
			territory.MQ:    "Martinique",
			territory.MR:    "Mauretanien",
			territory.MS:    "Montserrat",
			territory.MT:    "Malta",
			territory.MU:    "Mauritius",
			territory.MV:    "Maldiverne",
			territory.MW:    "Malawi",
			territory.MX:    "Mexico",
			territory.MY:    "Malaysia",
			territory.MZ:    "Mozambique",
			territory.NA:    "Namibia",
			territory.NC:    "Ny Kaledonien",
			territory.NE:    "Niger",
			territory.NF:    "Norfolk Island",
			territory.NG:    "Nigeria",
			territory.NI:    "Nicaragua",
			territory.NL:    "Nederlandene",
			territory.NO:    "Norge",
			territory.NP:    "Nepal",
			territory.NR:    "Nauru",
			territory.NU:    "Niue",
			territory.NZ:    "New Zealand",
			territory.OM:    "Oman",
			territory.PA:    "Panama",
			territory.PE:    "Peru",
			territory.PF:    "Fransk Polynesien",
			territory.PG:    "Papua Ny Guinea",
			territory.PH:    "Filippinerne",
			territory.PK:    "Pakistan",
			territory.PL:    "Polen",
			territory.PM:    "Saint Pierre og Miquelon",
			territory.PN:    "Pitcairn",
			territory.PR:    "Puerto Rico",
			territory.PS:    "De palæstinensiske områder",
			territory.PT:    "Portugal",
			territory.PW:    "Palau",
			territory.PY:    "Paraguay",
			territory.QA:    "Qatar",
			territory.QO:    "Ydre Oceanien",
			territory.RE:    "Réunion",
			territory.RO:    "Rumænien",
			territory.RS:    "Serbien",
			territory.RU:    "Rusland",
			territory.RW:    "Rwanda",
			territory.SA:    "Saudi-Arabien",
			territory.SB:    "Salomonøerne",
			territory.SC:    "Seychellerne",
			territory.SD:    "Sudan",
			territory.SE:    "Sverige",
			territory.SG:    "Singapore",
			territory.SH:    "St. Helena",
			territory.SI:    "Slovenien",
			territory.SJ:    "Svalbard og Jan Mayen",
			territory.SK:    "Slovakiet",
			territory.SL:    "Sierra Leone",
			territory.SM:    "San Marino",
			territory.SN:    "Senegal",
			territory.SO:    "Somalia",
			territory.SR:    "Surinam",
			territory.SS:    "Sydsudan",
			territory.ST:    "São Tomé og Príncipe",
			territory.SV:    "El Salvador",
			territory.SX:    "Sint Maarten",
			territory.SY:    "Syrien",
			territory.SZ:    "Eswatini",
			territory.TA:    "Tristan da Cunha",
			territory.TC:    "Turks- og Caicosøerne",
			territory.TD:    "Tchad",
			territory.TF:    "De Franske Besiddelser i Det Sydlige Indiske Ocean og Antarktis",
			territory.TG:    "Togo",
			territory.TH:    "Thailand",
			territory.TJ:    "Tadsjikistan",
			territory.TK:    "Tokelau",
			territory.TL:    "Timor-Leste",
			territory.TM:    "Turkmenistan",
			territory.TN:    "Tunesien",
			territory.TO:    "Tonga",
			territory.TR:    "Tyrkiet",
			territory.TT:    "Trinidad og Tobago",
			territory.TV:    "Tuvalu",
			territory.TW:    "Taiwan",
			territory.TZ:    "Tanzania",
			territory.UA:    "Ukraine",
			territory.UG:    "Uganda",
			territory.UM:    "Amerikanske oversøiske øer",
			territory.UN:    "De Forenede Nationer",
			territory.US:    "USA",
			territory.UY:    "Uruguay",
			territory.UZ:    "Usbekistan",
			territory.VA:    "Vatikanstaten",
			territory.VC:    "Saint Vincent og Grenadinerne",
			territory.VE:    "Venezuela",
			territory.VG:    "De Britiske Jomfruøer",
			territory.VI:    "De Amerikanske Jomfruøer",
			territory.VN:    "Vietnam",
			territory.VU:    "Vanuatu",
			territory.WF:    "Wallis og Futuna",
			territory.WS:    "Samoa",
			territory.XA:    "pseudo-accenter",
			territory.XB:    "pseudo-bidi",
			territory.XK:    "Kosovo",
			territory.YE:    "Yemen",
			territory.YT:    "Mayotte",
			territory.ZA:    "Sydafrika",
			territory.ZM:    "Zambia",
			territory.ZW:    "Zimbabwe",
			territory.ZZ:    "Ukendt område",
		},
	}
}
